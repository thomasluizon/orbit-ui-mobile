...
| `surface-manifest.mjs` | Derives the visual-surface inventory from the codebase (web routes, the multi-view Today root, overlays) into `.claude/manifests/surfaces.json`, expanded to one cell per surface x theme x locale. Emits **no status field** on purpose. | `npm run surfaces:manifest` |
| `capture-surfaces.mjs` | Playwright capture of one screenshot per manifest cell into `.artifacts/surfaces/`, against a running local stack. Reports every surface it cannot reach generically instead of skipping it. | `ORBIT_AUTH_TOKEN=… npm run surfaces:capture` |
| `check-surface-coverage.mjs` | The completion oracle: exit `0` only when every manifest cell has an artifact that exists, exceeds 5KB, and is newer than its `sourceFile`. Backs the `surface-coverage-gate` Stop hook. | `npm run surfaces:check` (`--json`) |
| `contact-sheet.mjs` | Composes every artifact into one self-contained HTML grid at `.artifacts/contact-sheet.html`, least-complete surfaces first, gaps rendered explicitly. | `npm run surfaces:sheet` |
| `drive-ab-report.mjs` | Per-model quality tally from the drive run logs (`.claude/drive/runs/**/task-*.json`): done/blocked/failed split, verifier DISAGREE rate, intervention rate, spend/duration, and a Sonnet−Opus comparison — so the Sonnet-vs-Opus quality delta on real bundles is measured, not guessed. Backs the `modelOverride` A/B in `/drive`. | `node tools/drive-ab-report.mjs [--since <date>] [--rework] [--json]` |
