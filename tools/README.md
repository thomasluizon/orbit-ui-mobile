...
| `record-readiness.mjs` | Re-reads the live PR base/head/draft and compare state, then persists and evaluates one repository-qualified receipt. READY requires CI, independent review, current connector review, zero threads, current base/head, non-draft state, live `behind_by=0`, and Linear synchronization simultaneously. Head/base changes produce explicit stale verdicts. In codex-only mode it reasserts the degraded first line at final aggregation. | `node tools/record-readiness.mjs --repo <key> --pr <n> --delivery <json> --review <json> --bot <json> --linear <json> [--codex-only]` |
