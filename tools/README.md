...
| `update-ticket.mjs` | Replaces one ticket body, the only writable copy of the work order a worker ever reads. Refuses without `--confirm-replace`, because the write overwrites every omitted section, and reports an identical or CRLF-twin body as unchanged without touching the issue. It never appends, comments, or moves labels, milestone or Status. | `node tools/update-ticket.mjs --issue <ORB-N\|#N\|N> --body-file <path\|-> --confirm-replace` |
