...
| `sync-linear-state.mjs` | Moves work/blocked/visual states to In Progress and simultaneous technical readiness to In Review, never Done. Stores the last posted state to suppress duplicate comments and emits a receipt for the readiness state machine. | `node tools/sync-linear-state.mjs --issue ORB-N --repo <key> --pr <n> --state <working\|blocked\|visual\|ready> --head-sha <sha> --base-sha <sha> --message-file <path\|->` |
