...
| `add-ticket-to-project.mjs` | Adds an existing issue to the configured project without changing its fields. It is an idempotent no-op when the card already exists, giving externally created tickets a sanctioned route into the completion workflow. | `node tools/add-ticket-to-project.mjs --issue <ORB-N\|#N\|N>` |
...
| `compose-prompt.mjs` | Builds one worker prompt with the ticket and comments, a brief, and one finishing contract. Both modes require an unchanged-test observation and a strengthened-test failure before fixing a missed regression, with evidence delivered to the PR body. Local workers push and open a PR; `--cloud` selects commit-only container delivery for the configured Cloud repository and carries evidence in handoff `testResults`. The orchestrator owns CI waiting in both modes. Refuses output inside a declared repo. | `node tools/compose-prompt.mjs --issue <ORB-N\|#N\|N> --repo <ui\|api\|landing> --out <abs path> [--cloud]` |
...
| `create-worktree.mjs` | Fetches and fast-forwards a requested local base to its remote commit before delegating worktree creation to Orca, and prints the exact selected commit. | `node tools/create-worktree.mjs --repo path:<repo> --name <slug> --base-branch <branch> --issue <ticket> --no-parent --comment <text> --json` |
