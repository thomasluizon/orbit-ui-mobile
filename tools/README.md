...
| `merge-sweep.sh` | Require-up-to-date server-side merge sweep: per PR, update-branch then poll `mergeStateStatus` until it is decidable and squash-merge. Skips on a failed required check or timeout. Blocks every merge until the `review` check on the CURRENT head SHA settles, so an update-branch cannot merge on a stale APPROVED, and exits 1 on a merged PR whose head branch was re-created. | `bash merge-sweep.sh <repo> <pr...>` (or `--help`) |
