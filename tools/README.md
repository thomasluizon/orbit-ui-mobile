...
| `lib/github-rate-limit.mjs` | Keeps the harness inside GitHub's per-user GraphQL budget and makes running out survivable. Decides proceed, wait, or refuse from a live rate-limit read, recognises the exhaustion message so it becomes a bounded wait instead of a dead run, and hands out a fixed number of global polling slots keyed by live pid so a killed poller cannot shrink the cap. `withGraphqlBudget` is the wrapper every GitHub-calling tool shares, so a ticket read backs off exactly like a poller does. | imported, not invoked |
