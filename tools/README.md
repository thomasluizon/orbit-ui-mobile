...
| `record-readiness.mjs` | Re-reads the live PR base/head/draft and compare state, then persists and evaluates one repository-qualified receipt. READY requires CI, independent review, current connector review, zero threads, current base/head, non-draft state, live `behind_by=0`, and Linear synchronization simultaneously. Head/base changes produce explicit stale verdicts. In codex-only mode it persists the pre-edit Guards baseline before reasserting the degraded first line and refuses READY until replacement runs register. | `node tools/record-readiness.mjs --repo <key> --pr <n> --delivery <json> --review <json> --bot <json> --linear <json> [--codex-only]` |
| `sync-linear-state.mjs` | Asserts the live ticket carries exactly the `repo:<key>` label matching `--repo` before either write, so a mistyped ticket key cannot move or comment on a stranger's ticket. Moves work/blocked/visual states to In Progress and simultaneous technical readiness to In Review, never Done. Stores the last posted state to suppress duplicate comments and emits a receipt for the readiness state machine. | `node tools/sync-linear-state.mjs --issue ORB-N --repo <key> --pr <n> --state <working\|blocked\|visual\|ready> --head-sha <sha> --base-sha <sha> --message-file <path\|->` |
