...
|---|---|---|
| `plan-queue.mjs` | Resolves a scope (explicit tickets, a Linear project, or the whole board) into ONE ordered execution plan: admission and deferrals, DAG-safe waves, transitive fan-out (`unlocks`), and stackable dependency chains. Tickets with independent same-repo blockers open against `main` in a later wave; cross-repo blockers also order but never stack. A still-open blocker outside the queue defers the ticket. It plans only, and writes nothing anywhere. | `node tools/plan-queue.mjs (--tickets ORB-1,ORB-2 \| --project <name> \| --board)` (`--format markdown`) |
