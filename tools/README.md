...
| `launch-worker.mjs` | Spawns one headless worker and supervises it in the foreground. Both clocks kill the complete process tree. Review mode requires an explicit repository and runs from that repository's primary main checkout; the review contract is single-sourced in this repository and materialized into the review order, so no cross-repo parity exists to assert. GitHub credentials are owner-selected per child environment; global account state is never switched. A codex-only post-worker PR-body edit persists the pre-edit Guards baseline first, so later delivery cannot reuse old green CI. | `node tools/launch-worker.mjs --issue <ORB-N\|#N\|N> --worktree <path> --prompt <file>` or `node tools/launch-worker.mjs --issue <ORB-N\|#N\|N> --review --repo <key> --prompt <file>` |
| `verify-delivery.mjs` | Derives delivery solely from bounded Git and GitHub children: clean/pushed/current PR, exact current-head CI state, and GitHub compare `behind_by=0`. `OUT_OF_DATE` reports base/head SHAs and behind count. File and line counts remain `sizeAdvisory` and never block. Completed CI passes only on the confirmed `SUCCESS`, `NEUTRAL`, or `SKIPPED` allowlist; every other or future conclusion fails closed. Failed CI retains run/check ID, details URL, workflow, name, status and conclusion; newest rerun wins. `--codex-only` mechanically restores the degraded first line after later body edits. | `node tools/verify-delivery.mjs --issue <ORB-N\|#N\|N> --worktree <p> --branch <b> --repo <key> [--codex-only]` |
| `record-readiness.mjs` | Reads the live PR base/head/draft, compare, connector, and ticket state ONCE, then persists and evaluates one repository-qualified receipt. READY requires CI, independent review, current connector review, zero threads, current base/head, non-draft state, live `behind_by=0`, and ticket synchronization simultaneously. Head/base changes produce explicit stale verdicts; the review's rubric snapshot path is recorded as information, never proven (the rubric is single-sourced and materialized by one code path). In codex-only mode it persists the pre-edit Guards baseline before reasserting the degraded first line and refuses READY until replacement runs register. | `node tools/record-readiness.mjs --repo <key> --pr <n> --delivery <json> --review <json> --bot <json> --ticket <json> [--codex-only]` |
