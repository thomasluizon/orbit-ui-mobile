...
| `check-i18n-usage.mjs` | Checks web, mobile and shared translation literals against both catalogs and enforces key parity, with no baseline. Prints counts and files for unresolved keys or translator namespaces (including translators passed through props/parameters). Parses local bindings, constant namespaces and server translators. Staged mode reads the index and scans all indexed sources when a catalog changes. | `node tools/check-i18n-usage.mjs [--staged] [--root <directory>]` |
