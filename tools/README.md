...
| `materialize-cloud-result.mjs` | Reads one task through `codex cloud list --json`, quarantines late results, and serially applies a ready diff at the receipt's exact base SHA. Recovery authenticates raw patch bytes. New submissions require a committed Cloud handoff, preserved in the receipt and returned on retries; exit 10 blocks missing metadata or a pending decision. It reports Git status and staged stat from disk. Authoritative empty-result handling remains distinct from delivery. | `node tools/materialize-cloud-result.mjs --receipt <file> [--allow-abandoned]` |
