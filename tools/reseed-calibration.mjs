...
const VERDICTS = {
  ...
  ".claude/skills/orchestrate/SKILL.md":
    "current: high effort, and it earns it: it plans the queue, enforces standing admission controls such as cloud.enabled, owns local post-materialization review work, verifies delivery from artifacts and clears the review. Reseeded 2026-09-18 a fourth time, when step 9 gained the third ending a pull request can reach. A run must now say which of READY, BLOCKED and merged its work landed on, and two of those are facts it writes down rather than verdicts it may assert: a merge commit sha, or a named blocker. The receipt names no field saying which required check failed, so the skill teaches a derivation instead, delivery's ci.checks.pass true beside the receipt's ci.green false, and the run has to hold that reading against a receipt that stays CI_STALE even after a successful merge. That sits on top of the calls the earlier stamps named: when an exhausted allowance justifies a transient worker switch, holding the revert and naming the switch while it is live, judging whether a substitute reviewer is named openly or promoted quietly, and telling an instructed contract apart from one a run granted itself. It now also reads four deliberately red gate assertions as expected while treating a fifth as a real defect, a count the previous stamp got wrong by verifying it against a narrowed gate. Each of those is the kind of call a lower effort rounds to the easy answer, and merge authority is the most expensive rule here to get wrong.",
