...
const VERDICTS = {
  ...
  ".claude/skills/progress/SKILL.md":
    "current: medium effort, because it reads live git and ticket state and must judge whether a part-built screen is honestly described, which low effort gets wrong by rounding up. The 2026-09-15 rewrite made it effort-agnostic and added --full, so it now resolves its own branch, spec and ticket meanings at answer time instead of reading them from a table here; that is more judgement per call, not less.",
