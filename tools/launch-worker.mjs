...
/** A per-launch override can raise the configured ceiling. */
...
const child = spawn(executable, workerArgs, {
  ...
  env: {
    ...
    ORCA_CLI_COMMAND: process.env.ORCA_BIN || (process.env.LOCALAPPDATA ? join(process.env.LOCALAPPDATA, "Programs", "orca", "resources", "bin", "orca") : "orca"),
  },
})
...
/** The supervisor pid is the wake source because the worker is its child. */
...
const finish = (outcome, exitCode) => {
  ...
  closeSync(logFd)
  /** Read the tree once so a stopped run reports any commits it left. */
