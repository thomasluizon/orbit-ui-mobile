...
const runEvidence = (nodes, expectedHead = HEAD, reviewsHaveNextPage = false, filesHaveNextPage = false, { ledgerPath = REVIEW_EVIDENCE_LEDGER, repository = "orbit/ui", pullRequest = 615, reviewAuthorityPublicKey = REVIEW_AUTHORITY_PUBLIC_KEY } = {}) =>
  spawnSync(process.execPath, [toolPath("check-review-evidence.mjs"), "--repository", repository, "--pull-request", String(pullRequest), "--expected-head", expectedHead, ...(reviewAuthorityPublicKey ? ["--review-authority-public-key", reviewAuthorityPublicKey] : [])], {
    encoding: "utf8",
...
const reviewEvidenceCases = () => {
  ...
  const missingAuthority = runEvidence([review({ body: MARKER(HEAD, "APPROVE") })], HEAD, false, false, { reviewAuthorityPublicKey: null })
  T("check-review-evidence.mjs: a missing retained review authority blocks closed", missingAuthority.status === 1 && /UNAUTHENTICATED/.test(missingAuthority.stdout), missingAuthority.stderr || missingAuthority.stdout)
  ...
  const attackerAuthority = generateKeyPairSync("ed25519")
  const attackerProvenance = issueReviewProvenance({
    repository: "orbit/ui",
    pullRequest: 615,
    head: HEAD,
    reviewNodeId: "PRR_attacker_authority",
    recommendation: "APPROVE",
    ledgerPath: join(root, "attacker-review.jsonl"),
    privateKey: attackerAuthority.privateKey,
  })
  const attackerMarker = `<!-- orbit-local-review: ${JSON.stringify({ version: 1, head: HEAD, recommendation: "APPROVE", provenance: attackerProvenance })} -->`
  const attackerEvidence = runEvidence([review({ body: attackerMarker })])
  T("check-review-evidence.mjs: a validly shaped review signed by a different authority fails against Sol's expected key", attackerEvidence.status === 1 && /UNAUTHENTICATED/.test(attackerEvidence.stdout), attackerEvidence.stderr || attackerEvidence.stdout)
  ...
  const badConnection = spawnSync(process.execPath, [toolPath("check-review-evidence.mjs"), "--repository", "orbit/ui", "--pull-request", "615", "--expected-head", HEAD, "--review-authority-public-key", REVIEW_AUTHORITY_PUBLIC_KEY], {
    encoding: "utf8",
  ...
  const movedSnapshot = spawnSync(process.execPath, [toolPath("check-review-evidence.mjs"), "--repository", "orbit/ui", "--pull-request", "615", "--expected-head", HEAD, "--review-authority-public-key", REVIEW_AUTHORITY_PUBLIC_KEY], {
    encoding: "utf8",
  ...
  const badInput = spawnSync(process.execPath, [toolPath("check-review-evidence.mjs"), "--repository", "orbit/ui", "--pull-request", "615", "--expected-head", HEAD, "--review-authority-public-key", REVIEW_AUTHORITY_PUBLIC_KEY], { encoding: "utf8", input: "not-json", env: { ...process.env, ORBIT_LOCAL_REVIEW_PROVENANCE_LEDGER: REVIEW_EVIDENCE_LEDGER } })
