...
const issueEnvelope = (state = { name: "In Progress", type: "started" }) => JSON.stringify({ id: "issue-read", ok: true, result: { issue: { identifier: "ORB-700", state, labels: [] } } })
...
export const cases = () => {
  ...
  const env = orcaEnv([
    { match: "linear issue ORB-700", stdout: issueEnvelope() },
  ...
  const duplicateEnv = orcaEnv([
    { match: "linear issue ORB-700", stdout: issueEnvelope({ name: "In Review", type: "started" }) },
