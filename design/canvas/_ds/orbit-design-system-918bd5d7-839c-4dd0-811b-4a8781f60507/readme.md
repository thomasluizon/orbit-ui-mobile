...
- **Status**: five of seven neutral. done = `--fg-1` disc + filled check; frozen = `--fg-2` + snowflake; skip = `--fg-3`; empty = `--track-empty` (`#717174` dark, `#87878B` light). Only overdue (dark `#FE9A00`; light `#886100` with white text) and error (dark `#FB2C36`; light `#E7000B`) carry a hue. Never colour alone: every status also carries an icon, shape, or label.
...
- **Closed 2026-09-10. fg-4 as an empty graphic above the canvas, dark and light.** The old binding measured 2.24 to 2.84 on dark raised surfaces and 2.94 on light hover. Thomas added `--track-empty` instead of collapsing the neutral status ranking. Dark `#717174` bottoms out at 3.024 on hover, and light `#87878B` bottoms out at 3.029 on hover. `--fg-4` remains unchanged and canvas-only for graphics.
