...
**The accent is warm orange `#C4530F`, granted by Thomas 2026-08-16, the only accent.** Rose is retired; there is no hue switch. To make orange legal in light mode, light `--status-overdue` first moved from `#B45B00` (9.8deg from the accent, inside the 15deg same-colour band) to `#946A00`. That value later proved too light for the well and hover surfaces, so Thomas moved it at constant OKLCH hue to `#8C6400` (hue 81.1, a 36.3deg gap; 5.11:1 on `#FAFAFA`, white on it 5.33:1). Dark `--status-overdue` is unchanged, and light `--fg-on-overdue` is `#FFFFFF`.
...
- **Status**: five of seven neutral. done = `--fg-1` disc + filled check; frozen = `--fg-2` + snowflake; skip = `--fg-3`; empty = `--fg-4`. Only overdue (dark `#FE9A00`; light `#8C6400` with white text) and error (dark `#FB2C36`; light `#E7000B`) carry a hue. Never colour alone: every status also carries an icon, shape, or label.
...
- **Light overdue text, CLOSED 2026-09-10.** `#946A00` measured 4.26 on the well, 4.11 on hover, and 4.31 on the widget well, below the 4.5 text floor. `#8C6400` now measures 4.68, 4.51, and 4.72 on those surfaces while preserving the OKLCH hue.
