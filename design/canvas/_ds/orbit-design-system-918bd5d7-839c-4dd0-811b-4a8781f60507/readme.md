...
**The accent is warm orange `#C4530F`, granted by Thomas 2026-08-16, the only accent.** Rose is retired; there is no hue switch. To make orange legal in light mode, light `--status-overdue` first moved from `#B45B00` (9.8deg from the accent, inside the 15deg same-colour band) to `#946A00`. That value later proved too light for the well and hover surfaces, so Thomas moved it at constant OKLCH hue to `#886100` (hue 81.1, a 36.3deg gap; 5.36:1 on `#FAFAFA`, white on it 5.59:1). Dark `--status-overdue` is unchanged, and light `--fg-on-overdue` is `#FFFFFF`.
...
- **Surfaces (dark)**: white alpha over the canvas, never lighter hexes: card .04, field .06, well .08, elev-2 .12, hover .13. Overlay panel is OPAQUE `#1C1C1E`. Hairlines: .08 / ghost .10 / strong .16. Scrim rgba(0,0,0,.55). Light: cards opaque `#FFFFFF`.
...
- **Status**: five of seven neutral. done = `--fg-1` disc + filled check; frozen = `--fg-2` + snowflake; skip = `--fg-3`; empty = `--track-empty` (`#7A7A7D` dark, `#7F7F83` light). Only overdue (dark `#FE9A00`; light `#886100` with white text) and error (dark `#FB2C36`; light `#E7000B`) carry a hue. Never colour alone: every status also carries an icon, shape, or label.
...
These pairs miss their WCAG floor. They are recorded here with their numbers because each trades against a rule Thomas set, so closing them is his call, not a silent pigment change. Item 1 (accent text on a raised surface, 4.28:1) was closed by scoping `--primary-soft` to the canvas. Limit 2 was closed on 2026-09-09 and its record is kept below.

- **Light overdue text, CLOSED 2026-09-10.** `#946A00` measured 4.26 on the well, 4.11 on hover, 4.31 on the widget well, and 4.12 on the 10 percent overdue tint the session-expiry warning uses, all below the 4.5 text floor. That tint is derived from the token, so it moves with it. `#886100` now measures 4.91, 4.73, 4.95 and 4.70 on those surfaces while preserving the OKLCH hue.
- **Limit 2, CLOSED 2026-09-09. fg-3 text on a hovered dark row: was 4.40:1, now 4.57:1.** The hover alpha went from .14 to .13, and .13 is the only value that satisfies both rules that bound it: at .14 fg-3 measured 4.39 against a 4.5 text floor, and at .12 the hover step fell to 1.229 against the 1.25:1 minimum a hover must clear over the surface it replaces, besides taking the exact value of elev-2. Read the paint order: one element swapping `--bg-card` for `--bg-hover` is a REPLACEMENT, and the canvas under `--bg-hover` is the stack, `#28282A`. A hover CHILD painted inside a card wrapper composites instead, `#313133`, where `--fg-3` measures 4.03 and misses the floor. The unread notification row is that second case, so text on a hover child inside a card is `--fg-2` (7.86) or lighter.
- **Closed 2026-09-10, corrected against composed states. fg-4 as an empty graphic above the canvas, dark and light.** The old binding measured 2.24 to 2.84 on dark raised surfaces and 2.94 on light hover. Thomas added `--track-empty` instead of collapsing the neutral status ranking. Dark `#7A7A7D` bottoms out at 3.034 on a hover child inside a card, and light `#7F7F83` bottoms out at 3.011 on selection over the canvas. Range endpoints apply their selection tint once. `--fg-4` remains unchanged and canvas-only for graphics.
