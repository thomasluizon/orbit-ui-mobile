...
- **Limit 2, CLOSED 2026-09-09. fg-3 text on a hovered dark row: was 4.40:1, now 4.57:1.** The hover alpha went from .14 to .13, and .13 is the only value that satisfies both rules that bound it: at .14 fg-3 measured 4.39 against a 4.5 text floor, and at .12 the hover step fell to 1.229 against the 1.25:1 minimum a hover must clear over the surface it replaces, besides taking the exact value of elev-2. Measure a hovered surface as a REPLACEMENT, not a layer: CSS `background-color` and React Native `backgroundColor` both swap the value, so a hovered card is the canvas under `--bg-hover` and no deeper stack exists to measure.
