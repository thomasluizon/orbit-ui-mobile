...
ruleTester.run('no-overshoot-easing', rule('no-overshoot-easing'), {
  valid: [
    ...
    'const e = [0.2, 0, 0, 1] as MotionBezier',
    'const e = [0.2, 0, 0, 1]',
    'const days = [16, 17, 18, 19]',
    'const values = [-0.1, 1.56, 0.64, 1]',
    'const values = [1.1, 1.56, 0.64, 1]',
    'const values = [0.34, 1.56, -0.1, 1]',
    'const values = [0.34, 1.56, 1.1, 1]',
    'const values = [16, 17, 18, 19] as const',
    'const values = [16, 17, 18, 19] as CalendarDays',
    'const e = [-1, 0, 2, 1] as MotionBezier',
    '<motion.div transition={{ ease: [0.16, 1, 0.3, 1] }} />',
    'Easing.bezier(0.4, 0, 1, 1)',
    'Easing.bezier(...easings.smooth)',
    'const e = [0.2, y1, 0, 1]',
    'const e = [0.2, , 0, 1]',
    'const e = [0.2, 2, 0]',
    'const e = [0.2, 2, 0, 1, 0]',
    'const e = ["0.2", "2", "0", "1"]',
    'const e = [0.2, ...controls, 0, 1]',
    'const e = [-1, 0, 2, 1]',
    'Animated.spring(value, { toValue: 1, damping: 26 })',
    '<motion.div initial={{ opacity: 0, scale: 0.25, filter: "blur(4px)" }} animate={{ opacity: 1, scale: 1, filter: "blur(0px)" }} transition={{ bounce: 0 }} />',
  ],
  invalid: [
    { code: 'const emphasize = [0.34, 1.56, 0.64, 1] as MotionBezier', errors: [{ messageId: 'noOvershoot' }] },
    { code: 'const e = [0.34, 1.56, 0.64, 1]', errors: [{ messageId: 'noOvershoot' }] },
    { code: 'const e = [0, 1.56, 1, 1]', errors: [{ messageId: 'noOvershoot' }] },
    { code: 'const e = [1, 0, 0, -0.5]', errors: [{ messageId: 'noOvershoot' }] },
    { code: 'const e = [-1, 1.56, 2, 1] as MotionBezier', errors: [{ messageId: 'noOvershoot' }] },
    { code: 'const e = [2, 0, -1, -0.5] as MotionBezier', errors: [{ messageId: 'noOvershoot' }] },
    { code: 'const e = "cubic-bezier(-1, 1.56, 2, 1)"', errors: [{ messageId: 'noOvershoot' }] },
    { code: 'Easing.bezier(-1, 1.56, 2, 1)', errors: [{ messageId: 'noOvershoot' }] },
    { code: '<motion.div transition={{ ease: [0.34, 1.56, 0.64, 1] }} />', errors: [{ messageId: 'noOvershoot' }] },
    { code: 'const e = [0.5, -0.5, 0.5, 1] as const', errors: [{ messageId: 'noOvershoot' }] },
    { code: 'const e = [0.5, 0, 0.5, -0.5]', errors: [{ messageId: 'noOvershoot' }] },
    { code: 'const e = [0.5, 0, 0.5, +1.5]', errors: [{ messageId: 'noOvershoot' }] },
    { code: 'Easing.bezier(0.34, 1.56, 0.64, 1)', errors: [{ messageId: 'noOvershoot' }] },
    { code: 'Easing.bezier(0.5, 0, 0.5, -0.5)', errors: [{ messageId: 'noOvershoot' }] },
    { code: 'Easing.bezier(...[0.34, 1.56, 0.64, 1])', errors: [{ messageId: 'noOvershoot' }] },
    { code: 'const e = `cubic-bezier(0.5, 0, 0.5, 1.5)`', errors: [{ messageId: 'noOvershoot' }] },
    { code: '<div className="ease-[cubic-bezier(0.34,_1.56,_0.64,_1)]" />', errors: [{ messageId: 'noOvershoot' }] },
...
ruleTester.run('spacing-scale', rule('spacing-scale'), {
  valid: [
    ...
    'const row = { gap: 14, padding: 16 }; ({ gap: row.gap } = runtimeStyle); <div style={row} />',
    'const row = { gap: 14, padding: 16 }; ({ nested: { gap: row.gap } } = runtimeStyle); <div style={row} />',
    'const row = { gap: 14, padding: 16 }; ({ [runtimeKey]: row.gap } = runtimeStyle); <div style={row} />',
    'const row = { gap: 14, padding: 16 }; [row.gap] = runtimeValues; <div style={row} />',
    'const row = { gap: 14, padding: 16 }; ([row.gap = runtimeGap] = runtimeValues); <div style={row} />',
    'const row = { gap: 14, padding: 16 }; [...row.gap] = runtimeValues; <div style={row} />',
    ...
    'const Object = { assign: () => ({}) }; const style = Object.assign({}, { gap: 14 }); <div style={style} />',
    'function Render({ Object }) { const style = Object.assign({}, { gap: 14 }); return <div style={style} /> }',
    'import { Object } from "./object"; const style = Object.assign({}, { gap: 14 }); <div style={style} />',
    'const Object = { assign: () => ({}) }; const row = {}; Object.assign(row, { gap: 14 }); <div style={row} />',
    ...
    'const row = { gap: 12 }; if (cond) { row.gap = 14; const marker = (row.gap = 16); } <div style={row} />',
    'const source = { gap: 12 }; if (cond) { delete source.gap; source.gap = 16; } const row = { gap: 14, ...source }; <div style={row} />',
    'const source = { gap: 12 }; if (cond) { delete source.gap; source.gap = 16; } const row = { gap: 14 }; Object.assign(row, source); <div style={row} />',
    'const row = { gap: 12 }; if (cond) { Object.assign(row, { gap: 14 }); Object.assign(row, { gap: 16 }); } <div style={row} />',
    'const row = { gap: 12 }; for (const item of items) { row.gap = 14; row.gap = 16; } <View style={row} />',
    'const row = { gap: 12 }; for (; cond; row.gap = 16) { row.gap = 14; } <View style={row} />',
    'const row = { gap: 12 }; switch (mode) { case 1: row.gap = 14; case 2: row.gap = 16; } <View style={row} />',
    'const row = { gap: 12 }; try { row.gap = 14; } catch (error) {} finally { row.gap = 16; } <View style={row} />',
    'const row = { gap: 12 }; cond && (row.gap = 14, row.gap = 16); <div style={row} />',
    'const row = { gap: 12 }; cond ? (row.gap = 14, row.gap = 16) : (row.gap = 8); <div style={row} />',
    'const row = { gap: 12 }; false && (row.gap = 14); <div style={row} />',
    'const row = { gap: 12 }; true ? (row.gap = 16) : (row.gap = 14); <div style={row} />',
    'const row = false && { gap: 14 }; <div style={row} />',
    'const row = { gap: 12 }; false ?? (row.gap = 14); <div style={row} />',
    ...
    'const row = [{ gap: 14 }, { ...{ gap: 12 } }]; <View style={row} />',
    'const row = {}; Object.assign(row, { gap: 14 }, { gap: 12 }); <div style={row} />',
    'const source = { gap: 14 }; source.gap = 12; const row = {}; Object.assign(row, source); <div style={row} />',
    'const source = { color: "red" }; source.gap = 12; const row = { gap: 14 }; Object.assign(row, source); <div style={row} />',
    'const source = { gap: 14 }; source.gap = 12; const row = {}; Object.assign(row, source, { gap: 16 }); <div style={row} />',
    'const row = getStyle(); <div style={row!} />',
    ...
    'const row = { gap: 14 }; labelled: { row.gap = 12; } <View style={row} />',
    'const row = { gap: 14 }; labelled: { delete row.gap; } <div style={row} />',
    'const base = { gap: 14 }; const row = { ...base, ...(active ? { gap: 12 } : { gap: 16 }) }; <div style={row} />',
  ...
  invalid: [
    ...
    { code: 'const row = { gap: 12 }; ({ ["gap"]: row.gap } = { gap: 14 }); <div style={row} />', output: null, errors: [{ messageId: 'offScaleStyle' }] },
    { code: 'const row = { gap: 12 }; [row.gap] = [14]; <div style={row} />', output: null, errors: [{ messageId: 'offScaleStyle' }] },
    { code: 'const row = { gap: 12 }; ([row.gap = 14] = []); <div style={row} />', output: null, errors: [{ messageId: 'offScaleStyle' }] },
    { code: 'const source = { gap: 14 }; const row = { gap: 12 }; ({ gap: row.gap } = source); <div style={row} />', output: null, errors: [{ messageId: 'offScaleStyle' }] },
    { code: 'const row = { gap: 12 }; ([{ gap: row.gap }] = [{ gap: 14 }]); <div style={row} />', output: null, errors: [{ messageId: 'offScaleStyle' }] },
    { code: 'const row = { gap: 12 }; switch (mode) { case 1: row.gap = 14; break; case 2: row.gap = 16; } <View style={row} />', output: null, errors: [{ messageId: 'offScaleStyle' }] },
    { code: 'const row = { gap: 12 }; cond && (row.gap = 14); <div style={row} />', output: null, errors: [{ messageId: 'offScaleStyle' }] },
    { code: 'const row = { gap: 12 }; cond ? (row.gap = 14) : (row.gap = 16); <div style={row} />', output: null, errors: [{ messageId: 'offScaleStyle' }] },
    { code: 'const row = { gap: 14 }; Object.assign(row, runtimeStyle); <div style={row} />', output: null, errors: [{ messageId: 'offScaleStyle' }] },
    { code: 'const getStyle = () => ({ gap: 12 }); const row = [{ gap: 14 }, getStyle()]; <View style={row} />', output: null, errors: [{ messageId: 'offScaleStyle' }] },
    { code: 'const row = { gap: 14 }; row[propertyName] = runtimeValue; <div style={row} />', output: null, errors: [{ messageId: 'offScaleStyle' }] },
    { code: 'const base = { gap: 14 }; const row = { ...base, ...runtimeStyle }; <div style={row} />', output: null, errors: [{ messageId: 'offScaleStyle' }] },
    { code: 'const base = { gap: 14 }; const row = { ...base, [dynamicKey]: 8 }; <div style={row} />', output: null, errors: [{ messageId: 'offScaleStyle' }] },
    {
      code: 'const source = { gap: 12 }; if (cond) delete source[propertyName]; const row = { gap: 14, ...source }; <div style={row} />',
      output: null,
      errors: [{ messageId: 'offScaleStyle' }],
    },
    {
      code: 'const source = {}; if (cond) source[propertyName] = 12; const row = { gap: 14, ...source }; <div style={row} />',
      output: null,
      errors: [{ messageId: 'offScaleStyle' }],
    },
    {
      code: 'const source = { gap: 12 }; if (cond) delete source[propertyName]; const row = { gap: 14 }; Object.assign(row, source); <div style={row} />',
      output: null,
      errors: [{ messageId: 'offScaleStyle' }],
    },
    {
      code: 'const row = { gap: 12 }; if (cond) { row.gap = 14; row.gap = 16; } else { row.gap = 20; } <div style={row} />',
      output: null,
      errors: [{ messageId: 'offScaleStyle' }],
    },
    {
    ...
    {
      code: 'const row = { gap: 14 }; <div style={row!} />',
      output: null,
      errors: [{ messageId: 'offScaleStyle' }],
    },
    {
      code: 'const row = { gap: 14 }; Object.assign(row, { color: "red" }); <div style={row} />',
      output: null,
      errors: [{ messageId: 'offScaleStyle' }],
    },
    {
      code: 'const row = { gap: 14, padding: 20 }; Object.assign(row, { gap: 12 }); <div style={row} />',
      output: null,
      errors: [{ messageId: 'offScaleStyle', data: { value: '20', prop: 'padding', scale: '0 4 8 12 16 24 32 48 64 96', nearest: '16' } }],
    },
    {
      code: 'const row = {}; Object.assign(row, { gap: 14 }); <div style={row} />',
      output: null,
      errors: [{ messageId: 'offScaleStyle' }],
    },
    {
      code: 'const source = { color: "red" }; source.gap = 14; const row = {}; Object.assign(row, source); <div style={row} />',
      output: null,
      errors: [{ messageId: 'offScaleStyle' }],
    },
    {
      code: 'const row = {}; Object.assign(row, { gap: 14 }, { padding: 20 }); <div style={row} />',
      output: null,
      errors: [{ messageId: 'offScaleStyle' }, { messageId: 'offScaleStyle' }],
    },
    {
      code: 'const source = { gap: 12 }; delete source.gap; const row = { gap: 14 }; Object.assign(row, source); <div style={row} />',
      output: null,
      errors: [{ messageId: 'offScaleStyle' }],
    },
    {
      code: 'const source = {}; source.gap = 12; delete source.gap; const row = { gap: 14 }; Object.assign(row, source); <div style={row} />',
      output: null,
      errors: [{ messageId: 'offScaleStyle' }],
    },
    {
      code: 'const source = { gap: 12 }; source.gap = 14; const row = {}; Object.assign(row, source); source.gap = 12; <div style={row} />',
      output: null,
      errors: [{ messageId: 'offScaleStyle' }],
    },
    {
      code: 'const row = [{ gap: 14 }, null, false]; <View style={row} />',
      output: null,
      errors: [{ messageId: 'offScaleStyle' }],
    },
    {
      code: 'const base = { gap: 14 }; const row = { ...base }; base.gap = 12; <div style={row} />',
      output: null,
      errors: [{ messageId: 'offScaleStyle' }],
    },
    {
      code: 'const row = [{ gap: 14, padding: 20 }, { gap: 12 }]; <View style={row} />',
      output: null,
      errors: [{ messageId: 'offScaleStyle', data: { value: '20', prop: 'padding', scale: '0 4 8 12 16 24 32 48 64 96', nearest: '16' } }],
    },
    {
    ...
    {
      code: 'const base = { gap: 14 }; const row = { ...base, ...(active ? { color: "red" } : { color: "blue" }) }; <div style={row} />',
      output: null,
      errors: [{ messageId: 'offScaleStyle', data: { value: '14', prop: 'gap', scale: '0 4 8 12 16 24 32 48 64 96', nearest: '12' } }],
    },
    {
      code: 'const base = { gap: 14 }; const row = { ...base, ...(active ? { gap: 12 } : { color: "blue" }) }; <div style={row} />',
      output: null,
      errors: [{ messageId: 'offScaleStyle', data: { value: '14', prop: 'gap', scale: '0 4 8 12 16 24 32 48 64 96', nearest: '12' } }],
    },
    {
