...
> - The authed-Today Lighthouse budget gate (`perf.yml`) uses a hermetic mock-api + fake-JWT harness to enforce LCP / TBT / script-bundle-size budgets on the signed-in Today surface at PR time (web-only, no prod, no secrets). LCP asserts the second-fastest of five runs; TBT and script size use the median. Its interactive twin is the `/profile` skill.
...
- **Thresholds from a measured baseline.** `apps/web/lighthouserc.json` runs 5 times. LHCI asserts median `total-blocking-time` and `resource-summary:script:size` at `error`. `apps/web/perf/assert-lcp-budget.mjs` discards the fastest LCP sample as one low runner outlier, then asserts the second-fastest at the unchanged 7800 ms budget. Three repeated batches on unchanged commit `67ff5542` produced second-fastest values of 7480, 7461, and 7430 ms. The ticket's noisy batch produced 7541 ms after trimming, while its 7855 ms median failed. A 400 ms regression makes every observed trimmed value exceed 7800; using the untrimmed best would let the 3197 ms outlier hide several seconds.
