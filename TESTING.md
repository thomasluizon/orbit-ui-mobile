...
| Harness hook parity | `.claude/hooks` | `node .claude/hooks/test-hooks.mjs` (no deps, local-only) | that the two surviving session hooks (git-guardrails, forbid-ef-migration-raw-index) block and allow exactly as their `_lib` rules specify, and that no agent's frontmatter carries a fails-open `Bash(...)` specifier |
...
- **`.github/workflows/test.yml`** - build, unit tests with coverage thresholds (`turbo run test -- --coverage`), type-check, lint, dependency-audit, design-guard, and contract-drift, on PRs to `main`. The hook suite (`node .claude/hooks/test-hooks.mjs`) is local-only since the Phase 3 teardown; run it after touching `.claude/hooks/` or an agent's frontmatter.
