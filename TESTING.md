...
Chromium geometry cases run inside Vitest with Playwright's `chrome` channel. Install Google Chrome before running the web suite. The GitHub-hosted `ubuntu-24.04` image satisfies this prerequisite. Every Chromium suite uses `__tests__/support/chromium.ts` to retain its in-flight launch through teardown, report a named launch failure within 30 seconds, and give its launch hook a separate 45 second budget. PillButton's target cases compile the actual Tailwind stylesheet and load the installed Geist font. These isolated component cases measure visible bounds, pseudo-element bounds and hit testing without an app server, network fixtures or production access; jsdom cannot resolve this layout.

The bad status role guard in `packages/shared/src/__tests__/theme.test.ts` walks production CSS and TypeScript under both apps on every run. It accepts direct `--status-bad` and `tokens.statusBad` references only when their syntax identifies a graphic or surface role, while named text consumers must use the text token. The committed snapshot keys each file with its reference count, so drift fails the shared suite without depending on source positions. After removing references, regenerate it with `npm run update:theme-inventory -w @orbit/shared`; its total count assertion prevents the ratchet from growing. Direct-reference discovery cannot infer a glyph that inherits its container color, so the mixed warning assertions keep that known case visible while #507 owns additional inherited-color shapes.
...
| Web perf budget (authed Today Lighthouse) | `apps/web` (`perf.yml`) | build web, boot the mock-api on `:5099`, then `API_BASE=http://127.0.0.1:5099 npm run perf -w @orbit/web` | that the signed-in Today surface (`/`) stays within median TBT / script-bundle-size budgets against the local mock orbit-api; the same five runs report the LCP distribution without gating |
...
- **Mock-BFF lever.** Every server-side upstream fetch resolves `process.env.API_BASE`, which is what lets that repoint work at all. An unmapped path logs and returns an empty body; promote it to a real fixture if a surface needs it.
...
- **Thresholds from a measured baseline.** `apps/web/lighthouserc.json` runs 5 times. LHCI asserts median `total-blocking-time` and `resource-summary:script:size` at `error`. LCP is reporting-only. Three unchanged batches on commit `67ff5542` spanned 6997 to 7632 ms, apart from the ticket's earlier 3197 ms low outlier. A clean trimmed-gate trial measured 6878 to 7534 ms. Adding a genuine 500 ms server-render delay then measured 6779 to 7626 ms, with a lower minimum and a median only 57 ms slower. No observed min, second-fastest, median, or 7800 ms threshold separates clean runs from that regression. `apps/web/perf/report-lcp.mjs` therefore prints all five LCP values plus minimum, median, and maximum without failing the job.
- **Run it locally.** From the repository root, set `API_BASE` to `http://127.0.0.1:5099` (`export API_BASE=http://127.0.0.1:5099` in Bash; `$env:API_BASE = 'http://127.0.0.1:5099'` in PowerShell), then run `npm run build -w @orbit/web`. Start the mock in a second terminal (`npx tsx apps/web/test-support/hermetic/mock-api/server.ts`), then run `npm run perf -w @orbit/web` in the first terminal. The perf script runs LHCI from the repository root, matching CI. Both write reports to root `.lighthouseci/`, which is gitignored and declared in `tools/root-allowlist.json`, so reports can remain on disk when committing. CI uploads that same directory as the `lighthouse-reports` artifact.
...
- **`.github/workflows/perf.yml`** - the authed-Today Lighthouse job, on PRs touching `apps/web/**` or `packages/shared/**` (pinned `ubuntu-24.04`, no secrets). Builds the web app, boots the hermetic mock orbit-api, and runs `lhci autorun` with a `puppeteerScript` fake-JWT injection. LHCI asserts median TBT and script size; the following step reports LCP from the same five reports. The signed-in `/` (Today) surface is the only measured URL. The job uploads `.lighthouseci/` reports.
- **Android widget host instrumented test** - local only. CI has no emulator, so an orchestrator runs it on a booted API 31+ device or emulator before merge.
