...
function StatusView({ status, label }: StatusViewProps) {
  ...
  const statusColors = {
    iconColor: status === 'failed' ? 'var(--status-bad)' : status === 'done' ? 'var(--fg-1)' : 'var(--fg-2)',
  }
  ...
  const row = (
    ...
    >
      <div className="flex min-w-0 flex-1 flex-col gap-1">
        ...
        {item.meta ? <div className={item.wrapLabel ? 'break-words text-xs text-[var(--fg-3)]' : 'truncate text-xs text-[var(--fg-3)]'}>{item.meta}</div> : null}
