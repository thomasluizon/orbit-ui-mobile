...
export function PublicProfileView({ view, t }: Readonly<PublicProfileViewProps>) {
  ...
  return (
    <div className="relative flex flex-col py-8" style={{ gap: 18 }}>
      <GradientTop height={260} />
      <header
        className="relative z-[1] flex flex-col items-center text-center"
        style={{ padding: '36px 24px 30px', gap: 10 }}
      >
        <span
          aria-hidden="true"
        ...
        <h1 style={{ margin: 0, fontFamily: 'var(--font-display)', fontSize: 26, fontWeight: 600, color: 'var(--fg-1)' }}>
          {view.displayName}
        </h1>
        {view.handle && (
      ...
      {view.achievements && view.achievements.length > 0 && (
        <section className="relative z-[1]" style={{ ...cardStyle, padding: '18px 18px' }}>
      ...
      <p className="relative z-[1] text-center" style={{ margin: 0, padding: '4px 0 12px', fontFamily: 'var(--font-sans)', fontSize: 12, color: 'var(--fg-4)' }}>
