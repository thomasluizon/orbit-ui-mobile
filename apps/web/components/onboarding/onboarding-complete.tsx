...
export function OnboardingComplete({ createdHabit, emoji, remindersOff, skipped, signedOut, dueToday, onFinish }: Readonly<OnboardingCompleteProps>) {
  const t = useTranslations('onboarding.flow.done')
  const { titleKey, bodyKey, pendingKey } = getOnboardingCompleteCopy({ skipped, signedOut, remindersOff, dueToday })
  return (
    <section className="mx-auto flex w-full max-w-md flex-col gap-6 py-8">
      ...
      {createdHabit ? <div className="flex items-center gap-4 rounded-[20px] bg-[var(--bg-card)] p-4 shadow-[inset_0_0_0_1px_var(--hairline-ghost)]"><span className="grid size-11 place-items-center rounded-[12px] bg-[var(--bg-well)] text-2xl">{emoji}</span><span className="min-w-0 flex-1"><strong className="block text-[var(--fg-1)]">{createdHabit}</strong><small className="text-[var(--fg-3)]">{t(pendingKey)}</small></span><StatusRing status="empty" size={30} label={t(pendingKey)} /></div> : null}
