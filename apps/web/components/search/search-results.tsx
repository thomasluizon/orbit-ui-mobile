...
export function SearchResults({ habits, query, onOpen }: Readonly<{ habits: readonly NormalizedHabit[]; query: string; onOpen: (id: string) => void }>) {
  const t = useTranslations()
  return <div className="flex flex-col gap-4">
    <p className="px-1 font-mono text-[length:var(--fs-xs)] text-[var(--fg-4)]">{t('habits.search.count', { count: habits.length })}</p>
