...
describe('Astra record list on web', () => {
  ...
  it('pages 10 of 37 in place and only shows supported destination chips', async () => {
    ...
    fireEvent.change(screen.getByRole('searchbox', { name: 'chat.recordList.filter' }), { target: { value: 'missing' } })
    expect(screen.getByText('chat.recordList.noMatches:{"query":"missing"}')).toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: 'chat.recordList.clearFilter' }))
