...
describe('goal server actions', () => {
  ...
  describe('createGoal', () => {
    ...
    it('includes auth headers', async () => {
      ...
      await createGoal({ title: 'Test Goal', targetValue: 100, unit: 'km' }, null)
    ...
    it('throws on non-OK response', async () => {
      ...
      await expect(createGoal({ title: '', targetValue: 0, unit: '' }, null)).rejects.toThrow('Title is required')
    })
  })
  ...
  describe('updateGoal', () => {
    it('sends PUT to /api/goals/:id', async () => {
      ...
      await updateGoal('goal-1', { title: 'Updated Goal', targetValue: 100, unit: 'km' }, null)
    ...
    it('throws on server error', async () => {
      ...
      await expect(updateGoal('nonexistent', { title: 'X', targetValue: 50, unit: 'km' }, null)).rejects.toThrow(
        'Goal not found',
      )
    })
  })
  ...
  describe('deleteGoal', () => {
    ...
    it('throws on server error', async () => {
      ...
      await expect(deleteGoal('nonexistent', null)).rejects.toThrow('Not found')
    })
  })
  ...
  describe('updateGoalProgress', () => {
    it('sends PUT to /api/goals/:id/progress', async () => {
      ...
      await updateGoalProgress('goal-1', { currentValue: 50 }, null)
    ...
    it('throws on non-OK response', async () => {
      ...
      await expect(
        updateGoalProgress('goal-1', { currentValue: -1 }, null),
  ...
  describe('updateGoalStatus', () => {
    it('sends PUT to /api/goals/:id/status', async () => {
      ...
      await updateGoalStatus('goal-1', { status: 'Completed' }, null)
    ...
    it('throws on server error', async () => {
      ...
      await expect(
        updateGoalStatus('goal-1', { status: 'invalid' as never }, null),
  ...
  describe('linkHabitsToGoal', () => {
    it('sends PUT to /api/goals/:id/habits', async () => {
      ...
      await linkHabitsToGoal('goal-1', ['h-1', 'h-2'], null)
    ...
    it('handles empty habit ids to unlink all', async () => {
      ...
      await linkHabitsToGoal('goal-1', [], null)
    ...
    it('throws on server error', async () => {
      ...
      await expect(
        linkHabitsToGoal('nonexistent', ['h-1'], null),
  ...
  describe('error handling', () => {
    it('throws with error message from response body', async () => {
      ...
      await expect(deleteGoal('nonexistent', null)).rejects.toThrow('Goal not found')
    })

    it('throws with message field from response body', async () => {
      mockApiResponse({ message: 'Validation failed' }, 400)

      await expect(createGoal({ title: '', targetValue: 0, unit: '' }, null)).rejects.toThrow('Validation failed')
    })
    ...
    it('throws with status code when no error body', async () => {
      ...
      await expect(deleteGoal('goal-1', null)).rejects.toThrow('500')
    })
  })
})
