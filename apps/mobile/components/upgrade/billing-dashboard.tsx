...
function PlanDetails({ status, data, locale, t, tokens }: Readonly<{
  ...
  return <PlanSummaryCard planLabel={t(summary.nameKey)}
    ...
    meta={[price, renewal].filter(Boolean).join(' · ')}
...
function InvoiceRow({ invoice, locale, state, t, tokens }: Readonly<{
  ...
  return (
    <View>
      ...
      {downloadFailed ? <Text accessibilityRole="alert" style={[styles.billingSecondary, { color: tokens.fg2 }]}>{t('upgrade.billing.portalFix')}</Text> : null}
    </View>
  )
}
