...
function buildUnderstandingSentence(
  ...
  if (days.length > 0) {
    const selectedDays = dayOptions.filter((day) => days.includes(day.value)).map((day) => day.label).join(', ')
    return translate('habits.form.understoodDays', { days: selectedDays })
  }
  if (isFlexible || hasFrequencyUnit) return translate('habits.form.understoodCount', { count: quantity })
  return null
}
