...
export function GoalLinkingField({ selectedGoalIds, atGoalLimit, onToggleGoal }: Readonly<GoalLinkingFieldProps>) {
  ...
  return (
    <>
      ...
      {open ? <Sheet ref={sheetRef} open title={t('habits.form.goals')} onClose={() => { setOpen(false); setQuery('') }}>
        {activeGoals.length === 0 ? <View style={styles.empty}><Text style={styles.emptyTitle}>{t('habits.form.noGoals')}</Text><Pressable accessibilityRole="button" style={styles.action} onPress={() => closeSheet(() => setShowCreateGoalModal(true))}><Text style={styles.actionText}>{t('habits.form.createGoal')}</Text></Pressable></View> : <View style={styles.list}>
          ...
          {activeGoals.length >= 21 ? <FlatList data={filteredGoals} renderItem={renderGoal} keyExtractor={(goal) => goal.id} style={styles.virtualList} initialNumToRender={8} windowSize={5} /> : filteredGoals.map((goal) => <View key={goal.id}>{renderGoal({ item: goal })}</View>)}
        </View>}
      </Sheet> : null}
    </>
  )
