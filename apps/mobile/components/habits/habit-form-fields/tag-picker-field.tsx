...
export function TagPickerField({ tags, selectedIds, atLimit, disabled, editor, onToggle, onCreate }: Readonly<TagPickerFieldProps>) {
  ...
  const renderTag = ({ item: tag }: { item: HabitTag }) => {
    ...
    return <Pressable accessibilityRole="button" accessibilityState={{ selected, disabled: disabled || (!selected && atLimit) }} disabled={disabled || (!selected && atLimit)} style={({ pressed }) => [styles.row, disabled || (!selected && atLimit) ? styles.disabled : null, pressed ? styles.pressed : null]} onPress={() => onToggle(tag.id)}><Text numberOfLines={1} style={styles.rowTitle}>{tag.name}</Text><Text style={styles.rowValue}>{selected ? '✓' : ''}</Text></Pressable>
  }
  return <>
    ...
    {open ? <Sheet open title={t('habits.form.tags')} onClose={() => { setOpen(false); setQuery('') }}><View style={styles.list}>
      ...
      {tags.length === 0 && !editor ? <View style={styles.empty}><Text style={styles.emptyTitle}>{t('habits.form.noTags')}</Text><Pressable accessibilityRole="button" style={styles.action} onPress={onCreate}><Text style={styles.actionText}>{t('habits.form.newTag')}</Text></Pressable></View> : null}
      ...
      {tags.length > 0 && !editor ? <Pressable accessibilityRole="button" style={styles.action} onPress={onCreate}><Text style={styles.actionText}>{t('habits.form.newTag')}</Text></Pressable> : null}
      {editor}
    </View></Sheet> : null}
  </>
}
...
function createStyles(tokens: Tokens) { return StyleSheet.create({
  chips: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, paddingTop: 8 }, chip: { backgroundColor: tokens.bgWell, borderRadius: 8, maxWidth: '100%', paddingHorizontal: 8, paddingVertical: 8 }, chipText: { color: tokens.fg2, flexShrink: 1, fontFamily: 'Rubik_500Medium', fontSize: 13 },
  list: { gap: 4 }, count: { color: tokens.fg3, fontFamily: 'Rubik_400Regular', fontSize: 12, padding: 8 }, search: { backgroundColor: tokens.bgField, borderColor: tokens.hairline, borderRadius: 12, borderWidth: 1, color: tokens.fg1, marginBottom: 8, minHeight: 44, paddingHorizontal: 12 }, virtualList: { maxHeight: 360 },
  row: { alignItems: 'center', borderRadius: 12, flexDirection: 'row', gap: 12, minHeight: 48, paddingHorizontal: 12 }, rowTitle: { color: tokens.fg1, flex: 1, fontFamily: 'Geist_400Regular', fontSize: 16 }, rowValue: { color: tokens.fg3, fontSize: 14 }, disabled: { opacity: 0.4 }, pressed: { backgroundColor: tokens.bgHover, transform: [{ scale: 0.96 }] },
  empty: { alignItems: 'center', gap: 16, padding: 32 }, emptyTitle: { color: tokens.fg1, fontFamily: 'Geist_500Medium', fontSize: 20, textAlign: 'center' }, action: { alignSelf: 'flex-start', backgroundColor: tokens.bgWell, borderRadius: 999, justifyContent: 'center', minHeight: 44, paddingHorizontal: 16 }, actionText: { color: tokens.fg1, fontFamily: 'Rubik_500Medium', fontSize: 14 },
}) }
