...
export function OnboardingFlow() {
  ...
  return <Modal visible animationType="none" onRequestClose={() => { if (step > 0) setStep(step - 1) }}><FlowShell nav={false} header={<FlowHeader step={step} onBack={() => setStep(step - 1)} onSkip={step === ONBOARDING_REMIND_STEP ? undefined : skip} />} action={<DecisionAction {...decisionProps} />} notice={createFailed ? <Toast kind="neutral" message={t('onboarding.flow.createFailed')} /> : undefined}><View style={styles.content}><DecisionContent {...decisionProps} /></View></FlowShell></Modal>
}
