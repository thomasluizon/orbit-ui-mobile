...
function IntervalCadence({ schedule, days, color, emphasis }: Readonly<{ schedule: OnboardingSchedule; days: string; color: string; emphasis: string }>) {
  ...
  return <Text style={[styles.sentence, { color }]}>{t('onboarding.flow.when.cadence.every')}{countPrefix}{t(`onboarding.flow.when.cadence.unit.${unit}.${unitCount}`)}{daysSuffix}<CadenceTime time={schedule.dueTime} at={t('onboarding.flow.when.cadence.at')} emphasis={emphasis} /></Text>
}
