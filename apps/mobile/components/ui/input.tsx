...
function Control({ props, tokens }: Readonly<{ props: InputProps; tokens: Tokens }>) {
  ...
  const borderColor = props.error ? tokens.statusBad : focused ? tokens.primary : tokens.borderControl
  useEffect(() => { if (props.focusRequest) inputRef.current?.focus() }, [props.focusRequest])
  return <View testID="input-control" style={[styles.control, { backgroundColor: tokens.bgField, borderColor, borderWidth: props.error || focused ? 2 : 1, outlineColor: tokens.primary, outlineWidth: focused ? 2 : 0 }, props.disabled ? styles.disabled : null]}>
