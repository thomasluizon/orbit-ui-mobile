...
function FreezeCoveredStatus({ date, remaining, origin, locale, tokens }: Readonly<{
  date: string
  remaining: number
  origin?: string | null
  locale: string
  ...
  const { t } = useTranslation()
  const message = t(origin === 'automatic' ? 'progressScreen.streak.automaticCovered' : 'progressScreen.streak.covered', {
...
function GoalCard({ goal, index, total, canReorder, isDragging, onDrag, onMove, onOpen, onRegister, tokens }: Readonly<{
  ...
  useEffect(() => {
    onRegister(goal.id, focusRef.current)
    return () => onRegister(goal.id, null)
  }, [goal.id, onRegister])
...
export function ProgressContent() {
  ...
  const registerGoalCard = useCallback((goalId: string, instance: View | null) => {
