...
function renderSelector(selectedEmoji = '', onSelect = vi.fn()) {
  let tree!: TestTree
  TestRenderer.act(() => {
...
describe('HabitEmojiSelector mobile', () => {
  ...
  it('filters, clears, and toggles a category in the picker', () => {
    ...
    TestRenderer.act(() => {
      ;(search.props.onChangeText as (value: string) => void)('not-an-emoji-query')
    })
    expect(tree.root.findAll(
      (node) => node.type === 'Text' && node.props.children === 'habits.form.emojiPickerEmpty',
    )).toHaveLength(1)
  ...
  it('selects and removes emoji values after the sheet closes', () => {
    ...
    const removal = renderSelector(firstEmoji)
    press(button(removal.tree, 'habits.form.emojiOpenPicker'))
    press(button(removal.tree, 'habits.form.emojiRemove'))
