...
function renderUnderstanding(overrides: Partial<React.ComponentProps<typeof HabitUnderstanding>> = {}) {
...
describe('HabitUnderstanding mobile', () => {
  it('shows the mirrored placeholder and forwards text entry while empty', () => {
    ...
    expect(tree.root.findAll(
      (node) => node.type === 'Text' && node.props.children === labels.placeholder,
    )).toHaveLength(1)
  ...
  it('renders consumed words, proposal copy, errors, and all correction controls', () => {
    ...
    expect(tree.root.findAll(
      (node) => node.type === 'Text' && node.props.children === 'A title is required',
    )[0]!.props.accessibilityRole).toBe('alert')
    expect(tree.root.findAll(
      (node) => node.type === 'Text' && node.props.children === labels.understoodAstra,
    )).toHaveLength(1)
    expect(tree.root.findAll(
      (node) => node.type === 'Text' && node.props.children === labels.unresolved,
    )).toHaveLength(1)
    expect(tree.root.findAll(
      (node) => node.type === 'HabitEmojiSelector' && node.props.selectedEmoji === '🏃',
    )).toHaveLength(1)
    ...
    ;(monday.props.style as (state: { pressed: boolean }) => unknown)({ pressed: true })
    ;(tuesday.props.style as (state: { pressed: boolean }) => unknown)({ pressed: false })
  ...
  it('clamps both ends of the weekly quantity correction', () => {
    ...
    const upper = renderUnderstanding({ value: 'Run', quantity: 7, sentence: 'Seven times a week' })
    TestRenderer.act(() => {
      ;(button(upper.tree, labels.more).props.onPress as () => void)()
    })
    expect(upper.props.onQuantityChange).toHaveBeenCalledWith(7)
    expect(upper.tree.root.findAll(
      (node) => node.type === 'Text' && node.props.children === 'Seven times a week',
    )).toHaveLength(1)
  })
})
