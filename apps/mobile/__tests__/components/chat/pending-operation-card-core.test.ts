...
function sentinelCount(renderer: ReactTestRenderer, sentinel: string): number {
  return renderer.root.findAll((node) => node.props['data-sentinel'] === sentinel).length
}
...
describe('SharedPendingOperationCard', () => {
  it('confirms a destructive operation and exposes its completed frame', async () => {
    ...
    await act(() => card.record.buttons[1]?.onClick())
    expect(card.record.confirmSheet?.open).toBe(true)
    await act(() => card.record.confirmSheet?.onCancel())
    expect(card.record.confirmSheet?.open).toBe(false)
    await act(() => card.record.buttons[1]?.onClick())
    await act(() => card.record.confirmSheet?.onConfirm())
    ...
    card.renderer.update(React.createElement(React.Fragment))
  })
  ...
  it('hands a prepared step-up operation to the platform verifier', async () => {
    ...
    await act(() => card.record.stepUp?.onAction())
    ...
    await act(() => verification?.onClose())
    expect(sentinelCount(card.renderer, 'card')).toBe(1)
    expect(sentinelCount(card.renderer, 'verification')).toBe(0)

    await act(() => card.record.stepUp?.onAction())
    await act(() => card.record.verification?.onCompleted('done'))
    expect(card.record.blockFrame).toMatchObject({
      items: [{ status: 'done' }],
    })
    card.renderer.update(React.createElement(React.Fragment))
