...
describe('pending operation card state', () => {
  ...
  it('prepares, closes, completes, and dismisses a step up operation', async () => {
    ...
    await act(() => state.current().closeStepUp())
    await act(() => state.current().setConfirmOpen(true))
    await act(() => state.current().dismiss())
  ...
  it('marks failed preparation and restores busy after preparation rejects', async () => {
    ...
    failedState.renderer.update(React.createElement(React.Fragment))
    ...
    rejectedState.renderer.update(React.createElement(React.Fragment))
  })
})
...
describe('pending operation step up verification', () => {
  it('submits the current code and completes a successful operation', async () => {
    ...
    await act(() => state.current().setCode('123456'))
  ...
  it('shows the returned error or the generic error without completing', async () => {
    ...
    explicitErrorState.renderer.update(React.createElement(React.Fragment))
    ...
    genericErrorState.renderer.update(React.createElement(React.Fragment))
  })
