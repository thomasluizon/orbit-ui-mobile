...
- **Never navigate from a sheet in the same tick as closing it, and never unmount a presented one.** Direct navigation from a presented TrueSheet requires the react-native-screens patch (https://sheet.lodev09.com/guides/navigation); this app ships without it, so finish native dismissal before navigation. Schedule the navigation with `useSheetExitAction` (`hooks/use-sheet-exit-action.ts`: `scheduleExitAction`, then close) and run it from `BottomSheetModal`'s `onDidDismiss` (pass `runExitAction`); never call `router.*` directly from sheet content or while `open` flips false. The all-Modals claim was an inference: this branch's `components/ui/anchored-menu.tsx` raced render-phase `setState` with queued async `setState` at mount (thomasluizon/orbit-tickets#134; fixed in thomasluizon/orbit-ui-mobile#1041). Apply D136: "Reproduce a device bug on the exact shipped build before calling it fixed." Reference wiring: `app/(tabs)/calendar.tsx`.
