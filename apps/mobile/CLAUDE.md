...
- **Never navigate from a sheet in the same tick as closing it, and never unmount a presented one.** TrueSheet + expo-router without the react-native-screens patch (we ship unpatched - https://sheet.lodev09.com/guides/navigation) wedges ALL RN Modals on Android release builds until process restart. `Sheet` owns the whole close path: take `{ sheetRef, closeSheet }` from `useSheetHost()` (`components/ui/sheet.tsx`), pass `ref={sheetRef}`, then call `closeSheet()` to close, or `closeSheet(action)` when something has to run after the sheet is gone. `onClose` fires only from the completed native dismissal, so never call it yourself and never flip the open state directly. Reference wiring: `app/(tabs)/calendar.tsx`.
