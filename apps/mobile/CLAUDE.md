...
- **The Play build cannot run on an x86_64 emulator, and that is deliberate.** `app.json`'s `buildArchs` is ARM-only, so the bundle carries no `lib/x86_64/`; SoLoader then fails to load `libreactnative.so` and the process dies in `MainApplication.onCreate`. Build for the emulator with `npm run android:apk:emulator`, which adds the ABI through Gradle's `-PreactNativeArchitectures` for that build alone. **Never add `x86_64` to `app.json`** - prebuild regenerates `gradle.properties` from it, so that ships the slice to Play. Full path: `/android-generate --emulator`.
- **Bottom-sheet modals use TrueSheet (`@lodev09/react-native-true-sheet`), NOT `@gorhom/bottom-sheet`.** gorhom's `present()` + portal silently no-op on the New Architecture (Fabric/Bridgeless) in release builds (rAF-gated mount never flushes). The shared wrapper is `components/ui/sheet.tsx` — the single styling seam for all sheets; sheet content uses core `ScrollView`/`TextInput`. Don't reintroduce gorhom.
- **Never navigate from a sheet in the same tick as closing it, and never unmount a presented one.** Direct navigation from a presented TrueSheet requires the react-native-screens patch (https://sheet.lodev09.com/guides/navigation); this app ships without it, so finish native dismissal before navigation. `Sheet` owns the whole close path: take `{ sheetRef, closeSheet }` from `useSheetHost()` (`components/ui/sheet.tsx`), pass `ref={sheetRef}`, then call `closeSheet()` to close, or `closeSheet(action)` when something has to run after the sheet is gone. A close that no tap started (an effect, or a finished operation) passes the action twice, `closeSheet(action, action)`: `dismiss()` can reject (`VIEW_NOT_FOUND` while the sheet still mounts), and the second action finishes the close anyway, because unmounting a `TrueSheet` drops and dismisses its native view (`TrueSheetView.onDropInstance`). That is the one sanctioned unmount of a presented sheet. `onClose` fires only from the completed native dismissal, so never call it yourself and never flip the open state directly. The all-Modals claim was an inference: `main`'s `anchored-menu.tsx`, absent from this branch, raced render-phase `setState` with queued async `setState` at mount (thomasluizon/orbit-tickets#134; fixed in thomasluizon/orbit-ui-mobile#1041). Apply D136: "Reproduce a device bug on the exact shipped build before calling it fixed." Reference wiring: `app/(tabs)/calendar.tsx`.
...
- **Android ships resizable and without an orientation lock.** Keep primary content readable in landscape and large windows, cap long-form columns, and use `useWindowDimensions` for render-time geometry. Never cache `Dimensions.get()` at module scope.
- **Edge-to-edge system bars come from `edgeToEdgeEnabled=true`.** The release fixes config plugin removes Expo's deprecated transparent system-bar color attributes after prebuild. Keep fixed headers and bottom bars on safe-area insets, and keep TrueSheet on automatic inset adjustment.
