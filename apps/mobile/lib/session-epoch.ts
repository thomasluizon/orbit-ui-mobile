...
/**
 * Moves the device on to the next session and tells every listener. A store reset reaches only what
 * a store holds, and the root layout never unmounts, so account-scoped state kept in a React hook
