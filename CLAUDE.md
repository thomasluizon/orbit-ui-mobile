...
- **The visual completion gate** makes "is this visual task done?" a computed fact. `tools/surface-manifest.mjs` derives the surface inventory from the codebase into `.claude/manifests/surfaces.json` (one cell per surface x theme x locale, **no status field**); `tools/capture-surfaces.mjs` screenshots each cell into `.artifacts/surfaces/`; `tools/check-surface-coverage.mjs` is the oracle — exit `0` only when every cell has an artifact that exists, exceeds 5KB, and is newer than its `sourceFile`; `tools/contact-sheet.mjs` composes them into one reviewable grid. `.claude/hooks/surface-coverage-gate.mjs` is the Stop hook that blocks a turn on a shortfall, **armed only while `.claude/manifests/ACTIVE` exists** (`touch` it when a visual pass starts, delete it when the pass lands) so it is inert in unrelated sessions. Completion is never a field a model writes — it is derived from files on disk. See `.claude/rules/visual-delivery.md`.
