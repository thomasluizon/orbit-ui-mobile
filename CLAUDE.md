...
| `.claude/skills/handoff/SKILL.md` | Spec-driven handoff: one living spec per effort under `.claude/specs/`, a short prompt that points at it, and a full in-flight inventory with a disposition per item. An attended `/handoff` ENDS the session; later requests go into `NEXT.md`. |
