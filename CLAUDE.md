...
- **`/next`** answers "what is the next ticket to execute?" and stops. It ranks launchable tickets by downstream reach and names ONE without launching it (gate: `.claude/hooks/forbid-raw-repo-tool-surfacing.mjs`; appeal inline per command). `/orchestrate ORB-N --single` runs it.
