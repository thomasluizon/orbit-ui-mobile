...
- **`/next`** answers "what is the next ticket to execute?" and stops there. Same DAG, read-only: it ranks the launchable set by how many tickets each one unblocks and names ONE recommendation, launching nothing. It is the supported way to ask that question, because `tools/` scripts are called by skills and agents, never typed at a prompt. `/orchestrate ORB-N --single` is what you run after it answers.
...
- Session hooks (`.claude/settings.json`) are the three rules no CI gate can enforce at act time: `git-guardrails` (protected main, no-verify, the worktree junction footgun), `forbid-ef-migration-raw-index` (idempotent raw index SQL in EF migrations), and `forbid-raw-linear-mutation` (a Linear WRITE goes through the orca CLI, never hand-rolled GraphQL; reads stay open, and `projectCreate`/`projectUpdate` are allowed raw because orca has no project write at all). Everything else the old hook fleet enforced migrated to the deterministic gates named under Code standards (D6), and the opencode dual-target engine is gone (D22). `node .claude/hooks/test-hooks.mjs` is the proof; run it after touching `.claude/hooks/` or an agent's frontmatter, and the Harness Execution job runs it again in CI.
...
| **The D1..D42 decision register** | NOT in this repo. It lives in the brain vault as `2 Areas/20-29 Orbit Engineering/Decisions/The Orbit workflow decision register (D1 to D42).md`. Ticket bodies, this file, `AGENTS.md`, the skills and several `tools/` headers cite decisions by number; that file is what they cite. `REBUILD.md` held it until 2026-07-25, when Phase 7 completed and the document was deleted as designed (D12). |
