...
| `.claude/skills/orchestrate/SKILL.md` | Ticket queue, worker, review and readiness contract; D89 caps, terminal Cloud handoff failures with staged work preserved, and Cloud test results carried into the PR body's Test evidence section. Its §5.4.1 owns the engine-fallback contract for an exhausted Codex allowance, including the transient `worker` edit and its revert, the expected calibration staleness, the ban on a subagent standing in for a worker, and the one case where a named substitute reviewer is allowed. |
