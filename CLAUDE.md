...
| `.claude/skills/orchestrate/SKILL.md` | Ticket queue, worker, review and readiness contract; D89 caps, terminal Cloud handoff failures with staged work preserved, and Cloud test results carried into the PR body's Test evidence section. Its §5.4.1 owns the engine-fallback contract for an exhausted Codex allowance, including the transient `worker` edit and its revert, the two gate reds that switch is expected to cause, the ban on a subagent standing in for a worker, and the named-substitute-reviewer rule that §5.4.1, step 8 and Hard prohibitions now state in one voice. |
