...
- **`/next`** answers "what is the next ticket to execute?" and stops there. Same DAG, read-only: it ranks the launchable set by how many tickets each one unblocks and names ONE recommendation, launching nothing (gate: `.claude/hooks/forbid-raw-repo-tool-surfacing.mjs`). `/orchestrate ORB-N --single` is what you run after it answers.
