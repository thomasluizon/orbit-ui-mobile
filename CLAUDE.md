...
- **The visual completion gate** makes "is this visual task done?" a computed fact, in two evidence stages per cell. `tools/surface-manifest.mjs` derives the surface inventory from the codebase into `.claude/manifests/surfaces.json` (one cell per surface x theme x locale, **no status field**); `tools/capture-surfaces.mjs` screenshots each cell into `.artifacts/surfaces/`; `tools/judge-surfaces.mjs` spawns fresh, read-only `claude -p` **independent vision judges** over the actual PNGs and records per-surface verdicts in `.artifacts/surfaces/verdicts.json`, hash-bound to the exact bytes judged; `tools/check-surface-coverage.mjs` is the oracle - exit `0` only when every cell has a fresh >5KB artifact AND a judge verdict of `transformed` whose recorded sha256 still matches the PNG (a screenshot of an unchanged page no longer passes - that hole is how b5 shipped at 5% and reported done); `tools/contact-sheet.mjs` composes the grid. `.claude/hooks/surface-coverage-gate.mjs` is the Stop hook that blocks a turn on a shortfall, **ARMED BY DEFAULT whenever `surfaces.json` exists** - the opt-in ACTIVE file is gone (four straight sessions proved a gate the agent must arm itself never gets armed). Disarm is human-only: Thomas creates `.claude/manifests/PAUSED` in his own terminal; `forbid-gate-tamper` (PreToolUse) blocks agents from creating PAUSED, hand-editing `surfaces.json`, or writing `verdicts.json` outside the judge tool. Completion is never a field a model writes - it is derived from files, hashes, and an independent judge. See `.claude/rules/visual-delivery.md`.
...
| `.claude/rules/` | The judgement tier: 47 standing rules no gate can check, themed by activity (review, debugging, planning, product/content, **visual-delivery**). Read the themed file when you start that activity; `README.md` indexes them. **`visual-delivery.md` is mandatory for any redesign/"apply the design"/whole-app visual task** — it makes completeness structural (surface inventory + per-surface screenshot artifacts + the adversarial `completeness-critic` close gate + the seed fixture). |
| `.claude/manifests/surfaces.json` | The DERIVED visual-surface inventory (routes + the multi-view Today root + overlays, x theme x locale). Generated by `tools/surface-manifest.mjs`; carries no status field by design. The expected denominator for the completion gate. |
