...
Quoting a file under `node_modules` in a finding? Run `node tools/check-dependency-edits.mjs` first and say it came back clean. A file whose mtime is later than its own package's `package.json` was written by something other than the install, and a plain `npm install` will not repair it: npm leaves a complete package alone. The repair is `rm -rf node_modules/<package>` plus an install.
