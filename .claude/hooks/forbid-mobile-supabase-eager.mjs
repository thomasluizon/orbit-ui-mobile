#!/usr/bin/env node
// PostToolUse adapter for the mobile Supabase lazy-client guard. Thin: logic in
...
import { checkMobileSupabaseLazy } from "./_lib/rules-source.mjs"

try {
  const input = readStdinJson()
  const filePath = filePathFrom(input)
  if (!filePath || !existsSync(filePath)) process.exit(0)
  const verdict = checkMobileSupabaseLazy(filePath, readFileSync(filePath, "utf8"))
  if (verdict?.block) {
