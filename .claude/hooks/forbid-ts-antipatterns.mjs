#!/usr/bin/env node
// PostToolUse adapter for the TS anti-pattern guard. Thin: logic in
...
import { checkTsAntipatterns } from "./_lib/rules-source.mjs"

try {
  const input = readStdinJson()
  const filePath = filePathFrom(input)
  if (!filePath || !existsSync(filePath)) process.exit(0)
  const verdict = checkTsAntipatterns(filePath, readFileSync(filePath, "utf8"))
  if (verdict?.block) {
