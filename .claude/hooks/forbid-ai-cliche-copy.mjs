#!/usr/bin/env node
// PostToolUse adapter for the AI-cliché-copy guard. Thin: logic in
...
import { checkAiClicheCopy } from "./_lib/rules-copy.mjs"

try {
  const input = readStdinJson()
  const verdict = checkAiClicheCopy(addedTextFrom(input?.tool_input ?? {}), filePathFrom(input))
  if (verdict?.block) {
