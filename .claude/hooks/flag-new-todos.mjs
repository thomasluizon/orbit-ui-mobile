#!/usr/bin/env node
// PostToolUse adapter for the untracked-workaround-marker guard. Thin: logic in
...
import { checkNewTodos } from "./_lib/rules-source.mjs"

try {
  const input = readStdinJson()
  const filePath = filePathFrom(input)
  if (!filePath || !existsSync(filePath)) process.exit(0)
  const verdict = checkNewTodos(filePath, readFileSync(filePath, "utf8"))
  if (verdict?.block) {
