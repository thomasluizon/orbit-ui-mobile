#!/usr/bin/env node
// PostToolUse adapter for the placeholder-content guard. Thin: logic in
...
import { checkPlaceholderContent } from "./_lib/rules-copy.mjs"

try {
  const input = readStdinJson()
  const verdict = checkPlaceholderContent(addedTextFrom(input?.tool_input ?? {}), filePathFrom(input))
  if (verdict?.block) {
