#!/usr/bin/env node
// PostToolUse adapter for the orbit-api timezone guard. Thin: logic in
...
import { checkCsharpTimezone } from "./_lib/rules-source.mjs"

try {
  const input = readStdinJson()
  const filePath = filePathFrom(input)
  if (!filePath || !existsSync(filePath)) process.exit(0)
  const verdict = checkCsharpTimezone(filePath, readFileSync(filePath, "utf8"))
  if (verdict?.block) {
