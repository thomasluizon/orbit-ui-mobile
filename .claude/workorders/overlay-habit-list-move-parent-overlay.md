---
surfaceId: overlay-habit-list-move-parent-overlay
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-habit-list-move-parent-overlay

## Goal

Bring `overlay-habit-list-move-parent-overlay` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/habits/habit-list/move-parent-overlay.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/habits/habit-list/move-parent-overlay.tsx` | `local/spacing-scale` | 3 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-habit-list-move-parent-overlay'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-habit-list-move-parent-overlay'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-habit-list-move-parent-overlay'`, and it is a veto
