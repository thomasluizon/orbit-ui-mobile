---
surfaceId: m-overlay-navigation-notification-bell
platform: mobile
kind: overlay
ownedFiles: 4
cells: 4
...
# Work order: m-overlay-navigation-notification-bell

## Goal

Bring `m-overlay-navigation-notification-bell` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/components/navigation/notification-bell.styles.ts`
- `apps/mobile/components/navigation/notification-bell.tsx`
- `apps/mobile/components/navigation/notification-detail-modal.tsx`
- `apps/mobile/components/navigation/notification-row.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/mobile/components/navigation/notification-bell.styles.ts` | `local/spacing-scale` | 7 |
| `apps/mobile/components/navigation/notification-detail-modal.tsx` | `local/spacing-scale` | 4 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-overlay-navigation-notification-bell'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-overlay-navigation-notification-bell'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-overlay-navigation-notification-bell'`, and it is a veto
