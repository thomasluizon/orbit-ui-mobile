---
surfaceId: m-overlay-ui-anchored-menu
platform: mobile
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: m-overlay-ui-anchored-menu

## Goal

Bring `m-overlay-ui-anchored-menu` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/components/ui/anchored-menu.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/mobile/components/ui/anchored-menu.tsx` | `local/spacing-scale` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-overlay-ui-anchored-menu'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-overlay-ui-anchored-menu'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-overlay-ui-anchored-menu'`, and it is a veto
