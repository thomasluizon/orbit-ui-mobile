---
surfaceId: overlay-ui-trial-expired-modal
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-ui-trial-expired-modal

## Goal

Bring `overlay-ui-trial-expired-modal` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/ui/trial-expired-modal.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/ui/trial-expired-modal.tsx` | `local/spacing-scale` | 4 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-ui-trial-expired-modal'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-ui-trial-expired-modal'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-ui-trial-expired-modal'`, and it is a veto
