---
surfaceId: overlay-habits-controls-menu
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-habits-controls-menu

## Goal

Bring `overlay-habits-controls-menu` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/habits/controls-menu.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-habits-controls-menu'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-habits-controls-menu'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-habits-controls-menu'`, and it is a veto
