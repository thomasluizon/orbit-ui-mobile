...
# Work order: route-retrospective

## Goal

Bring `route-retrospective` (`/retrospective`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(app)/retrospective/_components/locked-block.tsx`
- `apps/web/app/(app)/retrospective/_components/retrospective-dashboard.tsx`
- `apps/web/app/(app)/retrospective/_components/retrospective-empty-state.tsx`
- `apps/web/app/(app)/retrospective/_components/retrospective-locked-states.tsx`
- `apps/web/app/(app)/retrospective/_components/retrospective-no-data-state.tsx`
- `apps/web/app/(app)/retrospective/_components/retrospective-view.tsx`
- `apps/web/app/(app)/retrospective/page.tsx`

## Backlog A: enumerated and machine-COUNTED (the fix is still a judgement call)

16 suppressed DESIGN.md violation(s) in your owned files. These are real defects that
were measured and committed to the lint baseline, then never assigned to anyone.

| file | rule | count |
|---|---|---|
| `apps/web/app/(app)/retrospective/_components/retrospective-dashboard.tsx` | `local/spacing-scale` | 8 |
| `apps/web/app/(app)/retrospective/_components/retrospective-view.tsx` | `local/spacing-scale` | 5 |
| `apps/web/app/(app)/retrospective/_components/locked-block.tsx` | `local/spacing-scale` | 1 |
| `apps/web/app/(app)/retrospective/_components/retrospective-empty-state.tsx` | `local/spacing-scale` | 1 |
| `apps/web/app/(app)/retrospective/_components/retrospective-no-data-state.tsx` | `local/spacing-scale` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'route-retrospective'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'route-retrospective'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'route-retrospective'`, and it is a veto
