---
surfaceId: route-social-challenges-id
platform: web
kind: route
ownedFiles: 1
cells: 4
...
# Work order: route-social-challenges-id

## Goal

Bring `route-social-challenges-id` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(app)/social/challenges/[id]/page.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'route-social-challenges-id'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'route-social-challenges-id'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'route-social-challenges-id'`, and it is a veto
