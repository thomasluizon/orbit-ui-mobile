---
surfaceId: m-overlay-ui-push-prompt
platform: mobile
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: m-overlay-ui-push-prompt

## Goal

Bring `m-overlay-ui-push-prompt` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/components/ui/push-prompt.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/mobile/components/ui/push-prompt.tsx` | `local/spacing-scale` | 3 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-overlay-ui-push-prompt'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-overlay-ui-push-prompt'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-overlay-ui-push-prompt'`, and it is a veto
