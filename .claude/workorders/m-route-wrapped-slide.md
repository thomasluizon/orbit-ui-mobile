---
surfaceId: m-route-wrapped-slide
platform: mobile
kind: route
href: /wrapped-slide
ownedFiles: 1
...
# Work order: m-route-wrapped-slide

## Goal

Bring `m-route-wrapped-slide` (`/wrapped-slide`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/wrapped-slide.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-wrapped-slide'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-wrapped-slide'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-wrapped-slide'`, and it is a veto
