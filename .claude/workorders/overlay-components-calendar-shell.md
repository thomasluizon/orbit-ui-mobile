---
surfaceId: overlay-components-calendar-shell
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-components-calendar-shell

## Goal

Bring `overlay-components-calendar-shell` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(app)/calendar/_components/calendar-shell.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(app)/calendar/_components/calendar-shell.tsx` | `local/spacing-scale` | 6 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-components-calendar-shell'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-components-calendar-shell'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-components-calendar-shell'`, and it is a veto
