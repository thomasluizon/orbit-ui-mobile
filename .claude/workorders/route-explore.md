...
# Work order: route-explore

## Goal

Bring `route-explore` (`/explore`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(app)/explore/page.tsx` | `local/spacing-scale` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'route-explore'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'route-explore'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'route-explore'`, and it is a veto
