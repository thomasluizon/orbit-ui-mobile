...
# Work order: route-r-code

## Goal

Bring `route-r-code` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'route-r-code'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'route-r-code'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'route-r-code'`, and it is a veto
