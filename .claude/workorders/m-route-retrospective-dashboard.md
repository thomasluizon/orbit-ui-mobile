---
surfaceId: m-route-retrospective-dashboard
platform: mobile
kind: route
href: /retrospective-dashboard
ownedFiles: 1
cells: 4
mechanicalDebt: 0
pixelEvidence: none
...
# Work order: m-route-retrospective-dashboard

## Goal

Bring `m-route-retrospective-dashboard` (`/retrospective-dashboard`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/retrospective-dashboard.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-retrospective-dashboard'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-retrospective-dashboard'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-retrospective-dashboard'`, and it is a veto
