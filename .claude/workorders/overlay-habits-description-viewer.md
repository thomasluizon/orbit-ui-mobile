---
surfaceId: overlay-habits-description-viewer
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-habits-description-viewer

## Goal

Bring `overlay-habits-description-viewer` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/habits/description-viewer.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/habits/description-viewer.tsx` | `local/spacing-scale` | 2 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-habits-description-viewer'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-habits-description-viewer'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-habits-description-viewer'`, and it is a veto
