...
# Work order: route-u-slug

## Goal

Bring `route-u-slug` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(public)/u/[slug]/page.tsx`
- `apps/web/components/profile/public-profile-view.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/profile/public-profile-view.tsx` | `local/spacing-scale` | 13 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'route-u-slug'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'route-u-slug'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'route-u-slug'`, and it is a veto
