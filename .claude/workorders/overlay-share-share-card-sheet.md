---
surfaceId: overlay-share-share-card-sheet
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-share-share-card-sheet

## Goal

Bring `overlay-share-share-card-sheet` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/share/share-card-sheet.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/share/share-card-sheet.tsx` | `local/spacing-scale` | 4 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-share-share-card-sheet'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-share-share-card-sheet'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-share-share-card-sheet'`, and it is a veto
