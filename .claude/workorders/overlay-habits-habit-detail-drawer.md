---
surfaceId: overlay-habits-habit-detail-drawer
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-habits-habit-detail-drawer

## Goal

Bring `overlay-habits-habit-detail-drawer` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/habits/habit-detail-drawer.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-habits-habit-detail-drawer'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-habits-habit-detail-drawer'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-habits-habit-detail-drawer'`, and it is a veto
