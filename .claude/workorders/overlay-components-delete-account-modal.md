---
surfaceId: overlay-components-delete-account-modal
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-components-delete-account-modal

## Goal

Bring `overlay-components-delete-account-modal` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(app)/profile/_components/delete-account-modal.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(app)/profile/_components/delete-account-modal.tsx` | `local/spacing-scale` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-components-delete-account-modal'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-components-delete-account-modal'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-components-delete-account-modal'`, and it is a veto
