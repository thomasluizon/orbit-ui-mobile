...
# Work order: route-preferences

## Goal

Bring `route-preferences` (`/preferences`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(app)/preferences/_components/marketing-consent-section.tsx`
- `apps/web/app/(app)/preferences/_components/preference-labels.ts`
- `apps/web/app/(app)/preferences/_components/preference-settings-list.tsx`
- `apps/web/app/(app)/preferences/_components/push-notification-section.tsx`
- `apps/web/app/(app)/preferences/_components/use-preference-controls.ts`
- `apps/web/app/(app)/preferences/page.tsx`
- `apps/web/hooks/use-push-notification-preferences.ts`
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(app)/preferences/_components/push-notification-section.tsx` | `local/spacing-scale` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'route-preferences'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'route-preferences'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'route-preferences'`, and it is a veto
