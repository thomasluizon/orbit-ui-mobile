...
# Work order: m-route-preferences

## Goal

Bring `m-route-preferences` (`/preferences`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/preferences-labels.ts`
- `apps/mobile/app/preferences.tsx`
- `apps/mobile/app/use-preference-controls.ts`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-preferences'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-preferences'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-preferences'`, and it is a veto
