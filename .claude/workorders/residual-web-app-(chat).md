---
surfaceId: residual-web-app-(chat)
platform: web
kind: residual
ownedFiles: 1
cells: 0
...
# Work order: residual-web-app-(chat)
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(chat)/error.tsx` | `local/spacing-scale` | 2 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'residual-web-app-(chat)'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'residual-web-app-(chat)'` agrees.
