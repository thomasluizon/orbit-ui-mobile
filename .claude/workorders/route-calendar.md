...
# Work order: route-calendar

## Goal

Bring `route-calendar` (`/calendar`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/calendar/calendar-day-cell.tsx`
- `apps/web/components/calendar/calendar-day-detail.tsx`
- `apps/web/components/calendar/calendar-grid.tsx`
- `apps/web/components/calendar/calendar-load-error.tsx`
- `apps/web/components/calendar/calendar-range-view.tsx`
- `apps/web/components/calendar/calendar-stats.tsx`
- `apps/web/components/calendar/calendar-time-grid.tsx`
- `apps/web/components/calendar/calendar-week-view.tsx`
- `apps/web/components/calendar/show-recurring-toggle.tsx`
- `apps/web/components/calendar/use-agenda-day.ts`

## Backlog A: enumerated and machine-COUNTED (the fix is still a judgement call)

33 suppressed DESIGN.md violation(s) in your owned files. These are real defects that
were measured and committed to the lint baseline, then never assigned to anyone.

| file | rule | count |
|---|---|---|
| `apps/web/components/calendar/calendar-time-grid.tsx` | `local/spacing-scale` | 12 |
| `apps/web/components/calendar/calendar-day-detail.tsx` | `local/spacing-scale` | 9 |
| `apps/web/components/calendar/calendar-grid.tsx` | `local/spacing-scale` | 3 |
| `apps/web/app/(app)/calendar/page.tsx` | `local/spacing-scale` | 2 |
| `apps/web/components/calendar/calendar-agenda-view.tsx` | `local/spacing-scale` | 2 |
| `apps/web/components/calendar/calendar-load-error.tsx` | `local/spacing-scale` | 2 |
| `apps/web/components/calendar/calendar-range-view.tsx` | `local/spacing-scale` | 1 |
| `apps/web/components/calendar/calendar-stats.tsx` | `local/spacing-scale` | 1 |
| `apps/web/components/calendar/calendar-week-view.tsx` | `local/spacing-scale` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'route-calendar'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'route-calendar'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'route-calendar'`, and it is a veto
