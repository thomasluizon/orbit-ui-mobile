---
surfaceId: residual-mobile-components-social
platform: mobile
kind: residual
ownedFiles: 1
cells: 0
...
# Work order: residual-mobile-components-social
...
- `apps/mobile/components/social/social-entry-card.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/mobile/components/social/social-entry-card.tsx` | `local/spacing-scale` | 4 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'residual-mobile-components-social'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'residual-mobile-components-social'` agrees.
