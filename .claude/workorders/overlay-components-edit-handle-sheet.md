---
surfaceId: overlay-components-edit-handle-sheet
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-components-edit-handle-sheet

## Goal

Bring `overlay-components-edit-handle-sheet` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(app)/social/_components/edit-handle-sheet.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(app)/social/_components/edit-handle-sheet.tsx` | `local/spacing-scale` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-components-edit-handle-sheet'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-components-edit-handle-sheet'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-components-edit-handle-sheet'`, and it is a veto
