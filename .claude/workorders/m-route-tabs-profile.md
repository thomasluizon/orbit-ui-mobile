---
surfaceId: m-route-tabs-profile
platform: mobile
kind: route
href: /profile
ownedFiles: 13
...
# Work order: m-route-tabs-profile

## Goal

Bring `m-route-tabs-profile` (`/profile`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/(tabs)/profile/_components/edit-name-sheet.tsx`
- `apps/mobile/app/(tabs)/profile/_components/fresh-start-modal.tsx`
- `apps/mobile/app/(tabs)/profile/_components/profile-account-actions.tsx`
- `apps/mobile/app/(tabs)/profile/_components/profile-action-button.tsx`
- `apps/mobile/app/(tabs)/profile/_components/profile-identity.tsx`
- `apps/mobile/app/(tabs)/profile/_components/profile-section-entrance.ts`
- `apps/mobile/app/(tabs)/profile/_components/profile-sections.tsx`
- `apps/mobile/app/(tabs)/profile/_components/profile-stat-row.tsx`
- `apps/mobile/app/(tabs)/profile/_components/profile-styles.ts`
- `apps/mobile/app/(tabs)/profile/_components/profile-subscription-display.ts`
- `apps/mobile/app/(tabs)/profile/_components/use-data-export.ts`

## Backlog A: enumerated and machine-COUNTED (the fix is still a judgement call)

18 suppressed DESIGN.md violation(s) in your owned files. These are real defects that
were measured and committed to the lint baseline, then never assigned to anyone.

| file | rule | count |
|---|---|---|
| `apps/mobile/app/(tabs)/profile/_components/profile-styles.ts` | `local/spacing-scale` | 7 |
| `apps/mobile/app/(tabs)/profile/_components/delete-account-modal.tsx` | `local/spacing-scale` | 5 |
| `apps/mobile/app/(tabs)/profile/_components/fresh-start-modal.tsx` | `local/spacing-scale` | 4 |
| `apps/mobile/app/(tabs)/profile/_components/edit-name-sheet.tsx` | `local/spacing-scale` | 1 |
| `apps/mobile/app/(tabs)/profile/_components/profile-action-button.tsx` | `local/spacing-scale` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-tabs-profile'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-tabs-profile'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-tabs-profile'`, and it is a veto
