---
surfaceId: m-route-calendar-picker-section
platform: mobile
kind: route
href: /calendar-picker-section
ownedFiles: 1
cells: 4
mechanicalDebt: 0
pixelEvidence: none
...
# Work order: m-route-calendar-picker-section

## Goal

Bring `m-route-calendar-picker-section` (`/calendar-picker-section`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/calendar-picker-section.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-calendar-picker-section'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-calendar-picker-section'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-calendar-picker-section'`, and it is a veto
