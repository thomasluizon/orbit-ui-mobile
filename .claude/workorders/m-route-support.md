...
# Work order: m-route-support

## Goal

Bring `m-route-support` (`/support`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
| file | rule | count |
|---|---|---|
| `apps/mobile/app/support.tsx` | `local/spacing-scale` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-support'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-support'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-support'`, and it is a veto
