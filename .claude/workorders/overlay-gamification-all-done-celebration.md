---
surfaceId: overlay-gamification-all-done-celebration
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-gamification-all-done-celebration

## Goal

Bring `overlay-gamification-all-done-celebration` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/gamification/all-done-celebration.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/gamification/all-done-celebration.tsx` | `local/no-arbitrary-zindex` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-gamification-all-done-celebration'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-gamification-all-done-celebration'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-gamification-all-done-celebration'`, and it is a veto
