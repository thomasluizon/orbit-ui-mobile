---
surfaceId: overlay-app-today-shell
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-app-today-shell

## Goal

Bring `overlay-app-today-shell` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(app)/today-shell.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(app)/today-shell.tsx` | `local/spacing-scale` | 4 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-app-today-shell'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-app-today-shell'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-app-today-shell'`, and it is a veto
