...
# Work order: m-route-streak

## Goal

Bring `m-route-streak` (`/streak`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
| file | rule | count |
|---|---|---|
| `apps/mobile/app/streak.tsx` | `local/spacing-scale` | 5 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-streak'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-streak'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-streak'`, and it is a veto
