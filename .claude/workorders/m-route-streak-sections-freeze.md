---
surfaceId: m-route-streak-sections-freeze
platform: mobile
kind: route
href: /streak-sections-freeze
ownedFiles: 1
cells: 8
mechanicalDebt: 0
pixelEvidence: none
...
# Work order: m-route-streak-sections-freeze

## Goal

Bring `m-route-streak-sections-freeze` (`/streak-sections-freeze`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/streak-sections-freeze.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-streak-sections-freeze'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-streak-sections-freeze'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-streak-sections-freeze'`, and it is a veto
