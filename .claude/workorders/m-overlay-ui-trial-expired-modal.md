---
surfaceId: m-overlay-ui-trial-expired-modal
platform: mobile
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: m-overlay-ui-trial-expired-modal

## Goal

Bring `m-overlay-ui-trial-expired-modal` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
| file | rule | count |
|---|---|---|
| `apps/mobile/components/ui/trial-expired-modal.tsx` | `local/spacing-scale` | 6 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-overlay-ui-trial-expired-modal'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-overlay-ui-trial-expired-modal'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-overlay-ui-trial-expired-modal'`, and it is a veto
