---
surfaceId: m-overlay-ui-app-time-picker
platform: mobile
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: m-overlay-ui-app-time-picker

## Goal

Bring `m-overlay-ui-app-time-picker` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/components/ui/app-time-picker.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/mobile/components/ui/app-time-picker.tsx` | `local/spacing-scale` | 4 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-overlay-ui-app-time-picker'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-overlay-ui-app-time-picker'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-overlay-ui-app-time-picker'`, and it is a veto
