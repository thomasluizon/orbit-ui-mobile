...
# Work order: route-chat

## Goal

Bring `route-chat` (`/chat`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(chat)/chat/chat-empty-state.tsx`
- `apps/web/app/(chat)/chat/page.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'route-chat'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'route-chat'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'route-chat'`, and it is a veto
