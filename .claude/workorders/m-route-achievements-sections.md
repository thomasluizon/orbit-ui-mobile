---
surfaceId: m-route-achievements-sections
platform: mobile
kind: route
href: /achievements-sections
ownedFiles: 1
cells: 4
mechanicalDebt: 3
pixelEvidence: none
...
# Work order: m-route-achievements-sections

## Goal

Bring `m-route-achievements-sections` (`/achievements-sections`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/achievements-sections.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/mobile/app/achievements-sections.tsx` | `local/spacing-scale` | 3 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-achievements-sections'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-achievements-sections'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-achievements-sections'`, and it is a veto
