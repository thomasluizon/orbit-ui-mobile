---
surfaceId: overlay-ui-app-date-picker
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-ui-app-date-picker

## Goal

Bring `overlay-ui-app-date-picker` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/ui/app-date-picker.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/ui/app-date-picker.tsx` | `local/spacing-scale` | 4 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-ui-app-date-picker'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-ui-app-date-picker'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-ui-app-date-picker'`, and it is a veto
