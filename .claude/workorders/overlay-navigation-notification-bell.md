---
surfaceId: overlay-navigation-notification-bell
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-navigation-notification-bell

## Goal

Bring `overlay-navigation-notification-bell` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/navigation/notification-bell.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/navigation/notification-bell.tsx` | `local/spacing-scale` | 4 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-navigation-notification-bell'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-navigation-notification-bell'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-navigation-notification-bell'`, and it is a veto
