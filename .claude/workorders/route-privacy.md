...
# Work order: route-privacy

## Goal

Bring `route-privacy` (`/privacy`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(public)/privacy/page.tsx` | `local/spacing-scale` | 3 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'route-privacy'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'route-privacy'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'route-privacy'`, and it is a veto
