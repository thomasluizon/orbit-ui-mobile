---
surfaceId: overlay-components-cheer-composer
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-components-cheer-composer

## Goal

Bring `overlay-components-cheer-composer` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(app)/social/_components/cheer-composer.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(app)/social/_components/cheer-composer.tsx` | `local/spacing-scale` | 2 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-components-cheer-composer'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-components-cheer-composer'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-components-cheer-composer'`, and it is a veto
