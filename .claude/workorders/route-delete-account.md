---
surfaceId: route-delete-account
platform: web
kind: route
href: /delete-account
ownedFiles: 1
...
# Work order: route-delete-account

## Goal

Bring `route-delete-account` (`/delete-account`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(public)/delete-account/page.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(public)/delete-account/page.tsx` | `local/spacing-scale` | 2 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'route-delete-account'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'route-delete-account'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'route-delete-account'`, and it is a veto
