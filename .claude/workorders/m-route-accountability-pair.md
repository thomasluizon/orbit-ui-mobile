---
surfaceId: m-route-accountability-pair
platform: mobile
kind: route
href: /accountability-pair
ownedFiles: 1
cells: 4
mechanicalDebt: 7
pixelEvidence: none
...
# Work order: m-route-accountability-pair

## Goal

Bring `m-route-accountability-pair` (`/accountability-pair`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/accountability-pair.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/mobile/app/accountability-pair.tsx` | `local/spacing-scale` | 7 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-accountability-pair'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-accountability-pair'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-accountability-pair'`, and it is a veto
