---
surfaceId: residual-web-components-share
platform: web
kind: residual
ownedFiles: 1
cells: 0
...
# Work order: residual-web-components-share
...
- `apps/web/components/share/share-card.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/share/share-card.tsx` | `local/spacing-scale` | 10 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'residual-web-components-share'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'residual-web-components-share'` agrees.
