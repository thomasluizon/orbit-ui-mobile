...
| file | rule | count |
|---|---|---|
| `apps/web/app/global-error.tsx` | `local/spacing-scale` | 5 |
| `apps/web/app/not-found.tsx` | `local/spacing-scale` | 2 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'residual-web-app'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'residual-web-app'` agrees.
