---
surfaceId: residual-mobile-app-(tabs)-profile-_components
platform: mobile
kind: residual
ownedFiles: 1
cells: 0
...
# Work order: residual-mobile-app-(tabs)-profile-_components
...
- `apps/mobile/app/(tabs)/profile/_components/next-reward-carrot.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/mobile/app/(tabs)/profile/_components/next-reward-carrot.tsx` | `local/spacing-scale` | 4 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'residual-mobile-app-(tabs)-profile-_components'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'residual-mobile-app-(tabs)-profile-_components'` agrees.
