---
surfaceId: m-route-achievements
platform: mobile
kind: route
href: /achievements
ownedFiles: 1
...
# Work order: m-route-achievements

## Goal

Bring `m-route-achievements` (`/achievements`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/achievements.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/mobile/app/achievements.tsx` | `local/spacing-scale` | 4 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-achievements'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-achievements'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-achievements'`, and it is a veto
