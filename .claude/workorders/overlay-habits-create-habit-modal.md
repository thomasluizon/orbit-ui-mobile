---
surfaceId: overlay-habits-create-habit-modal
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-habits-create-habit-modal

## Goal

Bring `overlay-habits-create-habit-modal` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/habits/create-habit-modal.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-habits-create-habit-modal'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-habits-create-habit-modal'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-habits-create-habit-modal'`, and it is a veto
