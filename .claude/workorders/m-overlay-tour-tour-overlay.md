---
surfaceId: m-overlay-tour-tour-overlay
platform: mobile
kind: overlay
ownedFiles: 3
cells: 4
...
# Work order: m-overlay-tour-tour-overlay

## Goal

Bring `m-overlay-tour-tour-overlay` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/components/tour/tour-overlay.tsx`
- `apps/mobile/components/tour/tour-spotlight.tsx`
- `apps/mobile/components/tour/tour-tooltip.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/mobile/components/tour/tour-tooltip.tsx` | `local/spacing-scale` | 3 |
| `apps/mobile/components/tour/tour-tooltip.tsx` | `local/no-arbitrary-zindex` | 2 |
| `apps/mobile/components/tour/tour-spotlight.tsx` | `local/no-arbitrary-zindex` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-overlay-tour-tour-overlay'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-overlay-tour-tour-overlay'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-overlay-tour-tour-overlay'`, and it is a veto
