---
surfaceId: overlay-components-new-pair-flow
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-components-new-pair-flow

## Goal

Bring `overlay-components-new-pair-flow` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(app)/social/_components/new-pair-flow.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(app)/social/_components/new-pair-flow.tsx` | `local/spacing-scale` | 10 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-components-new-pair-flow'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-components-new-pair-flow'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-components-new-pair-flow'`, and it is a veto
