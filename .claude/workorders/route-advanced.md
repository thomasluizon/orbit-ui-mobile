...
# Work order: route-advanced

## Goal

Bring `route-advanced` (`/advanced`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(app)/advanced/page.tsx`
- `apps/web/app/actions/api-keys.ts`
- `apps/web/hooks/use-api-key-management.ts`
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(app)/advanced/page.tsx` | `local/spacing-scale` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'route-advanced'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'route-advanced'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'route-advanced'`, and it is a veto
