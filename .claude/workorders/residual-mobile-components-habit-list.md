---
surfaceId: residual-mobile-components-habit-list
platform: mobile
kind: residual
ownedFiles: 3
cells: 0
...
# Work order: residual-mobile-components-habit-list
...
- `apps/mobile/components/habit-list/empty-state.tsx`
- `apps/mobile/components/habit-list/move-parent-dialog.tsx`
- `apps/mobile/components/habit-list/styles.ts`

## Backlog A: enumerated and machine-COUNTED (the fix is still a judgement call)

23 suppressed DESIGN.md violation(s) in your owned files. These are real defects that
were measured and committed to the lint baseline, then never assigned to anyone.

| file | rule | count |
|---|---|---|
| `apps/mobile/components/habit-list/styles.ts` | `local/spacing-scale` | 14 |
| `apps/mobile/components/habit-list/move-parent-dialog.tsx` | `local/spacing-scale` | 7 |
| `apps/mobile/components/habit-list/empty-state.tsx` | `local/spacing-scale` | 2 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'residual-mobile-components-habit-list'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'residual-mobile-components-habit-list'` agrees.
