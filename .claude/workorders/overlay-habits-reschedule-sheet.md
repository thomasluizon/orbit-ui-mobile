---
surfaceId: overlay-habits-reschedule-sheet
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-habits-reschedule-sheet

## Goal

Bring `overlay-habits-reschedule-sheet` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/habits/reschedule-sheet.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/habits/reschedule-sheet.tsx` | `local/spacing-scale` | 8 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-habits-reschedule-sheet'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-habits-reschedule-sheet'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-habits-reschedule-sheet'`, and it is a veto
