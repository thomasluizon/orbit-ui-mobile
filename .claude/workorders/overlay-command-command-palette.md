---
surfaceId: overlay-command-command-palette
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-command-command-palette

## Goal

Bring `overlay-command-command-palette` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
| file | rule | count |
|---|---|---|
| `apps/web/components/command/command-palette.tsx` | `local/no-arbitrary-zindex` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-command-command-palette'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-command-command-palette'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-command-command-palette'`, and it is a veto
