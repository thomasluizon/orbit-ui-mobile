...
20 suppressed DESIGN.md violation(s) in your owned files. These are real defects that
were measured and committed to the lint baseline, then never assigned to anyone.

| file | rule | count |
|---|---|---|
| `apps/mobile/components/message-bubble.tsx` | `local/spacing-scale` | 5 |
| `apps/mobile/components/version-update-drawer.tsx` | `local/spacing-scale` | 5 |
| `apps/mobile/components/goal-card.tsx` | `local/spacing-scale` | 4 |
| `apps/mobile/components/upgrade-required-screen.tsx` | `local/spacing-scale` | 3 |
| `apps/mobile/components/bottom-sheet-modal.tsx` | `local/spacing-scale` | 2 |
| `apps/mobile/components/upgrade-required-screen.tsx` | `local/no-arbitrary-zindex` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'residual-mobile-components'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'residual-mobile-components'` agrees.
