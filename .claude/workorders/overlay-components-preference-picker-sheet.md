---
surfaceId: overlay-components-preference-picker-sheet
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-components-preference-picker-sheet

## Goal

Bring `overlay-components-preference-picker-sheet` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(app)/preferences/_components/preference-picker-sheet.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-components-preference-picker-sheet'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-components-preference-picker-sheet'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-components-preference-picker-sheet'`, and it is a veto
