---
surfaceId: overlay-referral-referral-drawer
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-referral-referral-drawer

## Goal

Bring `overlay-referral-referral-drawer` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/referral/referral-drawer.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/referral/referral-drawer.tsx` | `local/spacing-scale` | 7 |
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-referral-referral-drawer'`, and it is a veto
