...
# Work order: m-route-ai-settings

## Goal

Bring `m-route-ai-settings` (`/ai-settings`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/ai-settings.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-ai-settings'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-ai-settings'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-ai-settings'`, and it is a veto
