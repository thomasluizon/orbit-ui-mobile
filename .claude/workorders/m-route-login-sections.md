---
surfaceId: m-route-login-sections
platform: mobile
kind: route
href: /login-sections
ownedFiles: 1
...
# Work order: m-route-login-sections

## Goal

Bring `m-route-login-sections` (`/login-sections`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/login-sections.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-login-sections'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-login-sections'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-login-sections'`, and it is a veto
