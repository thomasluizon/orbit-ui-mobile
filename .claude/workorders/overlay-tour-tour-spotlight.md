---
surfaceId: overlay-tour-tour-spotlight
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-tour-tour-spotlight

## Goal

Bring `overlay-tour-tour-spotlight` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/tour/tour-spotlight.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/tour/tour-spotlight.tsx` | `local/no-arbitrary-zindex` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-tour-tour-spotlight'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-tour-tour-spotlight'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-tour-tour-spotlight'`, and it is a veto
