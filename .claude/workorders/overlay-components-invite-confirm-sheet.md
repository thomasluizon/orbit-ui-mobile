---
surfaceId: overlay-components-invite-confirm-sheet
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-components-invite-confirm-sheet

## Goal

Bring `overlay-components-invite-confirm-sheet` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(app)/social/_components/invite-confirm-sheet.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-components-invite-confirm-sheet'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-components-invite-confirm-sheet'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-components-invite-confirm-sheet'`, and it is a veto
