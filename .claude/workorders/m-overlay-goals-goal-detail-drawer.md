---
surfaceId: m-overlay-goals-goal-detail-drawer
platform: mobile
kind: overlay
ownedFiles: 12
cells: 4
...
# Work order: m-overlay-goals-goal-detail-drawer

## Goal

Bring `m-overlay-goals-goal-detail-drawer` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/components/goals/goal-detail-drawer.tsx`
- `apps/mobile/components/goals/goal-detail-drawer/goal-action-footer.tsx`
- `apps/mobile/components/goals/goal-detail-drawer/goal-ask-astra-button.tsx`
- `apps/mobile/components/goals/goal-detail-drawer/goal-detail-collections.tsx`
- `apps/mobile/components/goals/goal-detail-drawer/goal-load-error.tsx`
- `apps/mobile/components/goals/goal-detail-drawer/goal-progress-block.tsx`
- `apps/mobile/components/goals/goal-detail-drawer/goal-progress-form.tsx`
- `apps/mobile/components/goals/goal-detail-drawer/styles.ts`
- `apps/mobile/components/goals/goal-detail-drawer/use-goal-progress-form-state.ts`
- `apps/mobile/components/goals/goal-detail-drawer/use-goal-status-actions.ts`
- `apps/mobile/components/goals/goal-detail-sections.tsx`
- `apps/mobile/components/goals/goal-metrics-panel.tsx`

## Backlog A: enumerated and machine-COUNTED (the fix is still a judgement call)

12 suppressed DESIGN.md violation(s) in your owned files. These are real defects that
were measured and committed to the lint baseline, then never assigned to anyone.

| file | rule | count |
|---|---|---|
| `apps/mobile/components/goals/goal-detail-drawer/styles.ts` | `local/spacing-scale` | 6 |
| `apps/mobile/components/goals/goal-detail-sections.tsx` | `local/spacing-scale` | 4 |
| `apps/mobile/components/goals/goal-metrics-panel.tsx` | `local/spacing-scale` | 2 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-overlay-goals-goal-detail-drawer'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-overlay-goals-goal-detail-drawer'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-overlay-goals-goal-detail-drawer'`, and it is a veto
