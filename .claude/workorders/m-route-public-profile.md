---
surfaceId: m-route-public-profile
platform: mobile
kind: route
href: /public-profile
ownedFiles: 1
...
# Work order: m-route-public-profile

## Goal

Bring `m-route-public-profile` (`/public-profile`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/public-profile.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/mobile/app/public-profile.tsx` | `local/spacing-scale` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-public-profile'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-public-profile'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-public-profile'`, and it is a veto
