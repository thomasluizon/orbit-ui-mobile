---
surfaceId: overlay-referral-referral-prompt
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-referral-referral-prompt

## Goal

Bring `overlay-referral-referral-prompt` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/referral/referral-prompt.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-referral-referral-prompt'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-referral-referral-prompt'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-referral-referral-prompt'`, and it is a veto
