---
surfaceId: residual-web-components-referral
platform: web
kind: residual
ownedFiles: 1
cells: 0
...
# Work order: residual-web-components-referral
...
- `apps/web/components/referral/referral-card.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/referral/referral-card.tsx` | `local/spacing-scale` | 4 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'residual-web-components-referral'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'residual-web-components-referral'` agrees.
