---
surfaceId: overlay-gamification-level-up-overlay
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
Bring `overlay-gamification-level-up-overlay` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/gamification/level-up-overlay.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/gamification/level-up-overlay.tsx` | `local/no-arbitrary-zindex` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-gamification-level-up-overlay'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-gamification-level-up-overlay'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-gamification-level-up-overlay'`, and it is a veto
