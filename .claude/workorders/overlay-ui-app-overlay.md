---
surfaceId: overlay-ui-app-overlay
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-ui-app-overlay

## Goal

Bring `overlay-ui-app-overlay` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/ui/app-overlay.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/ui/app-overlay.tsx` | `local/spacing-scale` | 5 |
| `apps/web/components/ui/app-overlay.tsx` | `local/no-arbitrary-zindex` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-ui-app-overlay'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-ui-app-overlay'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-ui-app-overlay'`, and it is a veto
