---
surfaceId: overlay-ui-create-api-key-modal
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-ui-create-api-key-modal

## Goal

Bring `overlay-ui-create-api-key-modal` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/ui/create-api-key-modal.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/ui/create-api-key-modal.tsx` | `local/spacing-scale` | 7 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-ui-create-api-key-modal'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-ui-create-api-key-modal'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-ui-create-api-key-modal'`, and it is a veto
