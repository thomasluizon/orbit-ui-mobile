---
surfaceId: residual-mobile-components-navigation
platform: mobile
kind: residual
ownedFiles: 1
cells: 0
...
# Work order: residual-mobile-components-navigation
...
| file | rule | count |
|---|---|---|
| `apps/mobile/components/navigation/bottom-tab-bar.tsx` | `local/spacing-scale` | 9 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'residual-mobile-components-navigation'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'residual-mobile-components-navigation'` agrees.
