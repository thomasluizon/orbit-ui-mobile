...
# Work order: m-route-wrapped

## Goal

Bring `m-route-wrapped` (`/wrapped`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-wrapped'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-wrapped'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-wrapped'`, and it is a veto
