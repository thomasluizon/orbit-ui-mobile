---
surfaceId: m-overlay-habits-create-habit-modal
platform: mobile
kind: overlay
ownedFiles: 2
cells: 4
...
# Work order: m-overlay-habits-create-habit-modal

## Goal

Bring `m-overlay-habits-create-habit-modal` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/components/habits/create-habit-modal/sub-habit-editor.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/mobile/components/habits/create-habit-modal.tsx` | `local/spacing-scale` | 9 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-overlay-habits-create-habit-modal'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-overlay-habits-create-habit-modal'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-overlay-habits-create-habit-modal'`, and it is a veto
