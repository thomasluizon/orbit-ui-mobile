---
surfaceId: overlay-components-buddy-invite-row
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-components-buddy-invite-row

## Goal

Bring `overlay-components-buddy-invite-row` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(app)/social/_components/buddy-invite-row.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-components-buddy-invite-row'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-components-buddy-invite-row'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-components-buddy-invite-row'`, and it is a veto
