...
Bring `route-login` (`/login`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(auth)/login/code-step.tsx`
- `apps/web/app/(auth)/login/email-step.tsx`
- `apps/web/app/(auth)/login/login-atoms.tsx`
- `apps/web/app/(auth)/login/login-form-helpers.ts`
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(auth)/login/login-sections.tsx` | `local/spacing-scale` | 3 |
| `apps/web/app/(auth)/login/code-step.tsx` | `local/spacing-scale` | 1 |
| `apps/web/app/(auth)/login/email-step.tsx` | `local/spacing-scale` | 1 |
| `apps/web/app/(auth)/login/page.tsx` | `local/spacing-scale` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'route-login'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'route-login'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'route-login'`, and it is a veto
