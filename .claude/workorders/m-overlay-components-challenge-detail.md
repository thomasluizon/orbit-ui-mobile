---
surfaceId: m-overlay-components-challenge-detail
platform: mobile
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: m-overlay-components-challenge-detail

## Goal

Bring `m-overlay-components-challenge-detail` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/social/challenges/_components/challenge-detail.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/mobile/app/social/challenges/_components/challenge-detail.tsx` | `local/spacing-scale` | 4 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-overlay-components-challenge-detail'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-overlay-components-challenge-detail'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-overlay-components-challenge-detail'`, and it is a veto
