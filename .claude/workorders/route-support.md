...
# Work order: route-support

## Goal

Bring `route-support` (`/support`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(app)/support/_components/support-field.tsx`
- `apps/web/app/(app)/support/_components/support-form.tsx`
- `apps/web/app/(app)/support/_components/support-success-state.tsx`
- `apps/web/app/(app)/support/page.tsx`
- `apps/web/app/actions/support.ts`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'route-support'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'route-support'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'route-support'`, and it is a veto
