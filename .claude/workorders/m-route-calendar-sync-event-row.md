---
surfaceId: m-route-calendar-sync-event-row
platform: mobile
kind: route
href: /calendar-sync-event-row
ownedFiles: 1
cells: 4
mechanicalDebt: 0
pixelEvidence: none
...
# Work order: m-route-calendar-sync-event-row

## Goal

Bring `m-route-calendar-sync-event-row` (`/calendar-sync-event-row`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/calendar-sync-event-row.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-calendar-sync-event-row'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-calendar-sync-event-row'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-calendar-sync-event-row'`, and it is a veto
