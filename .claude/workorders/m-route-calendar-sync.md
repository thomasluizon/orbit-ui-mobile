---
surfaceId: m-route-calendar-sync
platform: mobile
kind: route
href: /calendar-sync
ownedFiles: 1
...
# Work order: m-route-calendar-sync

## Goal

Bring `m-route-calendar-sync` (`/calendar-sync`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/calendar-sync.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-calendar-sync'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-calendar-sync'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-calendar-sync'`, and it is a veto
