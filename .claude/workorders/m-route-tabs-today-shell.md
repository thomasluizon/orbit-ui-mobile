---
surfaceId: m-route-tabs-today-shell
platform: mobile
kind: route
href: /today-shell
ownedFiles: 1
...
# Work order: m-route-tabs-today-shell

## Goal

Bring `m-route-tabs-today-shell` (`/today-shell`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/(tabs)/today-shell.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/mobile/app/(tabs)/today-shell.tsx` | `local/spacing-scale` | 3 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-tabs-today-shell'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-tabs-today-shell'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-tabs-today-shell'`, and it is a veto
