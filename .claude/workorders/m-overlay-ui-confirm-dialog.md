---
surfaceId: m-overlay-ui-confirm-dialog
platform: mobile
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: m-overlay-ui-confirm-dialog

## Goal

Bring `m-overlay-ui-confirm-dialog` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/components/ui/confirm-dialog.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/mobile/components/ui/confirm-dialog.tsx` | `local/spacing-scale` | 6 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-overlay-ui-confirm-dialog'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-overlay-ui-confirm-dialog'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-overlay-ui-confirm-dialog'`, and it is a veto
