...
# Work order: m-route-advanced

## Goal

Bring `m-route-advanced` (`/advanced`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/advanced-api-keys.ts`
- `apps/mobile/app/advanced.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/mobile/app/advanced.tsx` | `local/spacing-scale` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-advanced'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-advanced'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-advanced'`, and it is a veto
