---
surfaceId: overlay-goals-edit-goal-modal
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-goals-edit-goal-modal

## Goal

Bring `overlay-goals-edit-goal-modal` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/goals/edit-goal-modal.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/goals/edit-goal-modal.tsx` | `local/spacing-scale` | 3 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-goals-edit-goal-modal'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-goals-edit-goal-modal'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-goals-edit-goal-modal'`, and it is a veto
