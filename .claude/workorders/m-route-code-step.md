...
# Work order: m-route-code-step

## Goal

Bring `m-route-code-step` (`/code-step`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-code-step'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-code-step'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-code-step'`, and it is a veto
