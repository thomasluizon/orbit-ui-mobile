---
surfaceId: m-overlay-habit-list-confirm-dialogs
platform: mobile
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: m-overlay-habit-list-confirm-dialogs

## Goal

Bring `m-overlay-habit-list-confirm-dialogs` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/components/habit-list/confirm-dialogs.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-overlay-habit-list-confirm-dialogs'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-overlay-habit-list-confirm-dialogs'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-overlay-habit-list-confirm-dialogs'`, and it is a veto
