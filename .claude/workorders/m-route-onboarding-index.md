---
surfaceId: m-route-onboarding-index
platform: mobile
kind: route
href: /
ownedFiles: 1
...
# Work order: m-route-onboarding-index

## Goal

Bring `m-route-onboarding-index` (`/`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-onboarding-index'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-onboarding-index'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-onboarding-index'`, and it is a veto
