---
surfaceId: residual-web-app-(chat)-chat
platform: web
kind: residual
ownedFiles: 1
cells: 0
...
# Work order: residual-web-app-(chat)-chat
...
- `apps/web/app/(chat)/chat/chat-composer-bar.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(chat)/chat/chat-composer-bar.tsx` | `local/spacing-scale` | 11 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'residual-web-app-(chat)-chat'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'residual-web-app-(chat)-chat'` agrees.
