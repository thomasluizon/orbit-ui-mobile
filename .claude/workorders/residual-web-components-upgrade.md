---
surfaceId: residual-web-components-upgrade
platform: web
kind: residual
ownedFiles: 6
cells: 0
...
# Work order: residual-web-components-upgrade
...
- `apps/web/components/upgrade/plan-card.tsx`
- `apps/web/components/upgrade/plan-comparison-cards.tsx`
- `apps/web/components/upgrade/play-billing-dashboard.tsx`
- `apps/web/components/upgrade/pricing-section.tsx`
- `apps/web/components/upgrade/usage-stats.tsx`

## Backlog A: enumerated and machine-COUNTED (the fix is still a judgement call)

50 suppressed DESIGN.md violation(s) in your owned files. These are real defects that
were measured and committed to the lint baseline, then never assigned to anyone.

| file | rule | count |
|---|---|---|
| `apps/web/components/upgrade/billing-dashboard.tsx` | `local/spacing-scale` | 26 |
| `apps/web/components/upgrade/pricing-section.tsx` | `local/spacing-scale` | 9 |
| `apps/web/components/upgrade/play-billing-dashboard.tsx` | `local/spacing-scale` | 5 |
| `apps/web/components/upgrade/plan-card.tsx` | `local/spacing-scale` | 4 |
| `apps/web/components/upgrade/plan-comparison-cards.tsx` | `local/spacing-scale` | 4 |
| `apps/web/components/upgrade/usage-stats.tsx` | `local/spacing-scale` | 2 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'residual-web-components-upgrade'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'residual-web-components-upgrade'` agrees.
