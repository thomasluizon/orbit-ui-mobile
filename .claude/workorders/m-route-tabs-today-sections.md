---
surfaceId: m-route-tabs-today-sections
platform: mobile
kind: route
href: /today-sections
ownedFiles: 1
...
# Work order: m-route-tabs-today-sections

## Goal

Bring `m-route-tabs-today-sections` (`/today-sections`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/(tabs)/today-sections.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-tabs-today-sections'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-tabs-today-sections'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-tabs-today-sections'`, and it is a veto
