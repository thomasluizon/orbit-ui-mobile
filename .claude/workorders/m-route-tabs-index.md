...
# Work order: m-route-tabs-index

## Goal

Bring `m-route-tabs-index` (`/`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
| file | rule | count |
|---|---|---|
| `apps/mobile/app/(tabs)/index.tsx` | `local/spacing-scale` | 3 |
| `apps/mobile/app/(tabs)/index.tsx` | `local/no-arbitrary-zindex` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-tabs-index'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-tabs-index'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-tabs-index'`, and it is a veto
