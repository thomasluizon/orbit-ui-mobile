---
surfaceId: m-route-retrospective-view
platform: mobile
kind: route
href: /retrospective-view
ownedFiles: 1
cells: 8
mechanicalDebt: 0
pixelEvidence: none
...
# Work order: m-route-retrospective-view

## Goal

Bring `m-route-retrospective-view` (`/retrospective-view`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/retrospective-view.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-retrospective-view'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-retrospective-view'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-retrospective-view'`, and it is a veto
