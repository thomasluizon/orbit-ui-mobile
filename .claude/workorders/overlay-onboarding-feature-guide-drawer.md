---
surfaceId: overlay-onboarding-feature-guide-drawer
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-onboarding-feature-guide-drawer

## Goal

Bring `overlay-onboarding-feature-guide-drawer` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/onboarding/feature-guide-drawer.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/onboarding/feature-guide-drawer.tsx` | `local/spacing-scale` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-onboarding-feature-guide-drawer'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-onboarding-feature-guide-drawer'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-onboarding-feature-guide-drawer'`, and it is a veto
