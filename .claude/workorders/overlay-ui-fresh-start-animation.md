---
surfaceId: overlay-ui-fresh-start-animation
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-ui-fresh-start-animation

## Goal

Bring `overlay-ui-fresh-start-animation` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/ui/fresh-start-animation.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-ui-fresh-start-animation'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-ui-fresh-start-animation'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-ui-fresh-start-animation'`, and it is a veto
