...
# Work order: route-profile

## Goal

Bring `route-profile` (`/profile`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(app)/profile/_components/profile-account-actions.tsx`
- `apps/web/app/(app)/profile/_components/profile-feature-sections.tsx`
- `apps/web/app/(app)/profile/_components/profile-header-bar.tsx`
- `apps/web/app/(app)/profile/_components/profile-identity-header.tsx`
- `apps/web/app/(app)/profile/_components/profile-modals.tsx`
- `apps/web/app/(app)/profile/_components/profile-nav-sections.tsx`
- `apps/web/app/(app)/profile/_components/profile-stat-tiles.tsx`
- `apps/web/app/(app)/profile/_components/profile-subscription-section.tsx`
- `apps/web/app/(app)/profile/_components/subscription-card.tsx`
- `apps/web/app/(app)/profile/_components/use-data-export.ts`
- `apps/web/app/(app)/profile/_components/use-profile-screen.ts`
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(app)/profile/_components/profile-stat-tiles.tsx` | `local/spacing-scale` | 6 |
| `apps/web/app/(app)/profile/_components/profile-identity-header.tsx` | `local/spacing-scale` | 2 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'route-profile'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'route-profile'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'route-profile'`, and it is a veto
