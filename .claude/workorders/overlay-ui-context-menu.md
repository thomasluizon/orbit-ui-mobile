---
surfaceId: overlay-ui-context-menu
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-ui-context-menu

## Goal

Bring `overlay-ui-context-menu` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
| file | rule | count |
|---|---|---|
| `apps/web/components/ui/context-menu.tsx` | `local/no-arbitrary-zindex` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-ui-context-menu'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-ui-context-menu'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-ui-context-menu'`, and it is a veto
