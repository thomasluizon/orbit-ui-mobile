---
surfaceId: m-route-tabs-calendar
platform: mobile
kind: route
href: /calendar
ownedFiles: 10
...
Bring `m-route-tabs-calendar` (`/calendar`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/(tabs)/calendar/_components/calendar-day-detail.tsx`
- `apps/mobile/app/(tabs)/calendar/_components/calendar-day-entry.tsx`
- `apps/mobile/app/(tabs)/calendar/_components/calendar-grid.tsx`
- `apps/mobile/app/(tabs)/calendar/_components/calendar-loading-bar.tsx`
- `apps/mobile/app/(tabs)/calendar/_components/calendar-range-view.tsx`
- `apps/mobile/app/(tabs)/calendar/_components/calendar-stats.tsx`
- `apps/mobile/app/(tabs)/calendar/_components/calendar-time-grid.tsx`
- `apps/mobile/app/(tabs)/calendar/_components/calendar-week-view.tsx`
- `apps/mobile/app/(tabs)/calendar/_components/show-recurring-toggle.tsx`

## Backlog A: enumerated and machine-COUNTED (the fix is still a judgement call)

21 suppressed DESIGN.md violation(s) in your owned files. These are real defects that
were measured and committed to the lint baseline, then never assigned to anyone.

| file | rule | count |
|---|---|---|
| `apps/mobile/app/(tabs)/calendar/_components/calendar-time-grid.tsx` | `local/spacing-scale` | 7 |
| `apps/mobile/app/(tabs)/calendar/_components/calendar-day-entry.tsx` | `local/spacing-scale` | 5 |
| `apps/mobile/app/(tabs)/calendar/_components/calendar-grid.tsx` | `local/spacing-scale` | 3 |
| `apps/mobile/app/(tabs)/calendar.tsx` | `local/spacing-scale` | 2 |
| `apps/mobile/app/(tabs)/calendar/_components/calendar-day-detail.tsx` | `local/spacing-scale` | 1 |
| `apps/mobile/app/(tabs)/calendar/_components/calendar-range-view.tsx` | `local/spacing-scale` | 1 |
| `apps/mobile/app/(tabs)/calendar/_components/calendar-stats.tsx` | `local/spacing-scale` | 1 |
| `apps/mobile/app/(tabs)/calendar/_components/calendar-week-view.tsx` | `local/spacing-scale` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-tabs-calendar'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-tabs-calendar'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-tabs-calendar'`, and it is a veto
