---
surfaceId: m-route-wrapped-cover
platform: mobile
kind: route
href: /wrapped-cover
ownedFiles: 1
...
# Work order: m-route-wrapped-cover

## Goal

Bring `m-route-wrapped-cover` (`/wrapped-cover`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/wrapped-cover.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-wrapped-cover'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-wrapped-cover'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-wrapped-cover'`, and it is a veto
