---
surfaceId: overlay-habits-bulk-action-bar-v2
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-habits-bulk-action-bar-v2

## Goal

Bring `overlay-habits-bulk-action-bar-v2` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/habits/bulk-action-bar-v2.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/habits/bulk-action-bar-v2.tsx` | `local/spacing-scale` | 2 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-habits-bulk-action-bar-v2'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-habits-bulk-action-bar-v2'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-habits-bulk-action-bar-v2'`, and it is a veto
