---
surfaceId: m-route-retrospective-empty-state
platform: mobile
kind: route
href: /retrospective-empty-state
ownedFiles: 1
cells: 8
mechanicalDebt: 0
pixelEvidence: none
...
# Work order: m-route-retrospective-empty-state

## Goal

Bring `m-route-retrospective-empty-state` (`/retrospective-empty-state`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/retrospective-empty-state.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-retrospective-empty-state'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-retrospective-empty-state'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-retrospective-empty-state'`, and it is a veto
