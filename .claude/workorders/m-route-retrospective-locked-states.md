---
surfaceId: m-route-retrospective-locked-states
platform: mobile
kind: route
href: /retrospective-locked-states
ownedFiles: 1
cells: 4
mechanicalDebt: 0
pixelEvidence: none
...
# Work order: m-route-retrospective-locked-states

## Goal

Bring `m-route-retrospective-locked-states` (`/retrospective-locked-states`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/retrospective-locked-states.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-retrospective-locked-states'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-retrospective-locked-states'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-retrospective-locked-states'`, and it is a veto
