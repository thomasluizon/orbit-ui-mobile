---
surfaceId: residual-web-components-navigation
platform: web
kind: residual
ownedFiles: 1
cells: 0
...
# Work order: residual-web-components-navigation
...
| file | rule | count |
|---|---|---|
| `apps/web/components/navigation/app-sidebar.tsx` | `local/spacing-scale` | 8 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'residual-web-components-navigation'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'residual-web-components-navigation'` agrees.
