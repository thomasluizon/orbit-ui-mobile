---
surfaceId: m-route-wrapped-player
platform: mobile
kind: route
href: /wrapped-player
ownedFiles: 1
...
# Work order: m-route-wrapped-player

## Goal

Bring `m-route-wrapped-player` (`/wrapped-player`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/wrapped-player.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-wrapped-player'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-wrapped-player'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-wrapped-player'`, and it is a veto
