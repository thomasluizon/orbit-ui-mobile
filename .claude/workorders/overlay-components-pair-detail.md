---
surfaceId: overlay-components-pair-detail
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-components-pair-detail

## Goal

Bring `overlay-components-pair-detail` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(app)/social/_components/pair-detail.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(app)/social/_components/pair-detail.tsx` | `local/spacing-scale` | 7 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-components-pair-detail'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-components-pair-detail'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-components-pair-detail'`, and it is a veto
