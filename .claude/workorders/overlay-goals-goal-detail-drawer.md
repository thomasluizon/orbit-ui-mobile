---
surfaceId: overlay-goals-goal-detail-drawer
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-goals-goal-detail-drawer

## Goal

Bring `overlay-goals-goal-detail-drawer` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/goals/goal-detail-drawer.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/goals/goal-detail-drawer.tsx` | `local/spacing-scale` | 3 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-goals-goal-detail-drawer'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-goals-goal-detail-drawer'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-goals-goal-detail-drawer'`, and it is a veto
