---
surfaceId: m-overlay-components-friend-row
platform: mobile
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: m-overlay-components-friend-row

## Goal

Bring `m-overlay-components-friend-row` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/social/_components/friend-row.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/mobile/app/social/_components/friend-row.tsx` | `local/spacing-scale` | 8 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-overlay-components-friend-row'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-overlay-components-friend-row'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-overlay-components-friend-row'`, and it is a veto
