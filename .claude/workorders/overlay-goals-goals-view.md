---
surfaceId: overlay-goals-goals-view
platform: web
kind: overlay
ownedFiles: 1
cells: 8
...
# Work order: overlay-goals-goals-view

## Goal

Bring `overlay-goals-goals-view` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/goals/goals-view.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/goals/goals-view.tsx` | `local/spacing-scale` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-goals-goals-view'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-goals-goals-view'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-goals-goals-view'`, and it is a veto
