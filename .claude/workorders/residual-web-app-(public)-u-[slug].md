---
surfaceId: residual-web-app-(public)-u-[slug]
platform: web
kind: residual
ownedFiles: 1
cells: 0
...
# Work order: residual-web-app-(public)-u-[slug]
...
- `apps/web/app/(public)/u/[slug]/not-found.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(public)/u/[slug]/not-found.tsx` | `local/spacing-scale` | 2 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'residual-web-app-(public)-u-[slug]'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'residual-web-app-(public)-u-[slug]'` agrees.
