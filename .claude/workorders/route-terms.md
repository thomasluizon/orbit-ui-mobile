...
# Work order: route-terms

## Goal

Bring `route-terms` (`/terms`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(public)/terms/page.tsx` | `local/spacing-scale` | 3 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'route-terms'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'route-terms'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'route-terms'`, and it is a veto
