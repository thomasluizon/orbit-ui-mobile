---
surfaceId: m-route-streak-sections
platform: mobile
kind: route
href: /streak-sections
ownedFiles: 1
cells: 4
mechanicalDebt: 0
pixelEvidence: none
...
# Work order: m-route-streak-sections

## Goal

Bring `m-route-streak-sections` (`/streak-sections`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/streak-sections.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-streak-sections'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-streak-sections'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-streak-sections'`, and it is a veto
