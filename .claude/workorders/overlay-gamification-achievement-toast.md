---
surfaceId: overlay-gamification-achievement-toast
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-gamification-achievement-toast

## Goal

Bring `overlay-gamification-achievement-toast` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/gamification/achievement-toast.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/gamification/achievement-toast.tsx` | `local/spacing-scale` | 2 |
| `apps/web/components/gamification/achievement-toast.tsx` | `local/no-arbitrary-zindex` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-gamification-achievement-toast'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-gamification-achievement-toast'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-gamification-achievement-toast'`, and it is a veto
