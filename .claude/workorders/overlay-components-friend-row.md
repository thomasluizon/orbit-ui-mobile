---
surfaceId: overlay-components-friend-row
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-components-friend-row

## Goal

Bring `overlay-components-friend-row` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(app)/social/_components/friend-row.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(app)/social/_components/friend-row.tsx` | `local/spacing-scale` | 5 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-components-friend-row'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-components-friend-row'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-components-friend-row'`, and it is a veto
