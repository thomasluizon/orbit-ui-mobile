---
surfaceId: m-overlay-habits-edit-habit-modal
platform: mobile
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: m-overlay-habits-edit-habit-modal

## Goal

Bring `m-overlay-habits-edit-habit-modal` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-overlay-habits-edit-habit-modal'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-overlay-habits-edit-habit-modal'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-overlay-habits-edit-habit-modal'`, and it is a veto
