---
surfaceId: overlay-advanced-advanced-sections
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-advanced-advanced-sections

## Goal

Bring `overlay-advanced-advanced-sections` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/advanced/advanced-sections.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/advanced/advanced-sections.tsx` | `local/spacing-scale` | 6 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-advanced-advanced-sections'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-advanced-advanced-sections'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-advanced-advanced-sections'`, and it is a veto
