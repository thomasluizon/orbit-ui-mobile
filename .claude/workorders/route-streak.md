...
# Work order: route-streak

## Goal

Bring `route-streak` (`/streak`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(app)/streak/_components/streak-frozen-banner.tsx`
- `apps/web/app/(app)/streak/_components/streak-hero.tsx`
- `apps/web/app/(app)/streak/_components/streak-sections.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(app)/streak/_components/streak-sections.tsx` | `local/spacing-scale` | 19 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'route-streak'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'route-streak'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'route-streak'`, and it is a veto
