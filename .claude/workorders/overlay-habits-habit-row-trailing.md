---
surfaceId: overlay-habits-habit-row-trailing
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-habits-habit-row-trailing

## Goal

Bring `overlay-habits-habit-row-trailing` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/habits/habit-row-trailing.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/habits/habit-row-trailing.tsx` | `local/spacing-scale` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-habits-habit-row-trailing'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-habits-habit-row-trailing'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-habits-habit-row-trailing'`, and it is a veto
