...
# Work order: route-insights

## Goal

Bring `route-insights` (`/insights`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/charts/chart-ring.tsx`
- `apps/web/components/charts/multi-month-heatmap.tsx`
- `apps/web/components/charts/trend-line.tsx`
- `apps/web/components/insights/achievements-timeline-section.tsx`
- `apps/web/components/insights/completion-trends-section.tsx`
- `apps/web/components/insights/goal-progress-section.tsx`
- `apps/web/components/insights/insights-headline-model.ts`
- `apps/web/components/insights/insights-headline.tsx`
- `apps/web/components/insights/insights-locked-state.tsx`
- `apps/web/components/insights/insights-section-status.ts`
- `apps/web/components/insights/insights-section.tsx`
- `apps/web/components/insights/monthly-heatmap-section.tsx`
- `apps/web/components/insights/multi-habit-comparison-section.tsx`
- `apps/web/components/insights/range-selector.tsx`
- `apps/web/components/insights/range.ts`
- `apps/web/components/insights/streak-history-section.tsx`
- `apps/web/components/insights/xp-over-time-section.tsx`
- `apps/web/hooks/use-goal-progress-history.ts`
- `apps/web/hooks/use-habit-trends.ts`
- `apps/web/hooks/use-streak-history.ts`
- `apps/web/hooks/use-xp-history.ts`

## Backlog A: enumerated and machine-COUNTED (the fix is still a judgement call)

10 suppressed DESIGN.md violation(s) in your owned files. These are real defects that
were measured and committed to the lint baseline, then never assigned to anyone.

| file | rule | count |
|---|---|---|
| `apps/web/components/insights/insights-locked-state.tsx` | `local/spacing-scale` | 3 |
| `apps/web/components/charts/trend-line.tsx` | `local/spacing-scale` | 2 |
| `apps/web/app/(app)/insights/page.tsx` | `local/spacing-scale` | 1 |
| `apps/web/components/charts/bar-chart.tsx` | `local/spacing-scale` | 1 |
| `apps/web/components/charts/chart-ring.tsx` | `local/spacing-scale` | 1 |
| `apps/web/components/charts/multi-month-heatmap.tsx` | `local/spacing-scale` | 1 |
| `apps/web/components/insights/insights-headline.tsx` | `local/spacing-scale` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'route-insights'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'route-insights'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'route-insights'`, and it is a veto
