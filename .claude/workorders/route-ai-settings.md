...
# Work order: route-ai-settings

## Goal

Bring `route-ai-settings` (`/ai-settings`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(app)/ai-settings/_components/ai-feature-toggles.tsx`
- `apps/web/app/(app)/ai-settings/_components/fact-item.tsx`
- `apps/web/app/(app)/ai-settings/_components/facts-pagination.tsx`
- `apps/web/app/(app)/ai-settings/_components/facts-select-bar.tsx`
- `apps/web/app/(app)/ai-settings/_components/pro-upgrade-link.tsx`
- `apps/web/app/(app)/ai-settings/_components/use-user-facts.ts`
- `apps/web/app/(app)/ai-settings/_components/user-facts-list.tsx`
- `apps/web/app/(app)/ai-settings/page.tsx`
- `apps/web/app/actions/user-facts.ts`

## Backlog A: enumerated and machine-COUNTED (the fix is still a judgement call)

6 suppressed DESIGN.md violation(s) in your owned files. These are real defects that
were measured and committed to the lint baseline, then never assigned to anyone.

| file | rule | count |
|---|---|---|
| `apps/web/app/(app)/ai-settings/page.tsx` | `local/spacing-scale` | 2 |
| `apps/web/app/(app)/ai-settings/_components/fact-item.tsx` | `local/spacing-scale` | 1 |
| `apps/web/app/(app)/ai-settings/_components/facts-pagination.tsx` | `local/spacing-scale` | 1 |
| `apps/web/app/(app)/ai-settings/_components/facts-select-bar.tsx` | `local/spacing-scale` | 1 |
| `apps/web/app/(app)/ai-settings/_components/user-facts-list.tsx` | `local/spacing-scale` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'route-ai-settings'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'route-ai-settings'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'route-ai-settings'`, and it is a veto
