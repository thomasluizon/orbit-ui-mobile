---
surfaceId: overlay-goals-goal-card
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-goals-goal-card

## Goal

Bring `overlay-goals-goal-card` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/goals/goal-card.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/goals/goal-card.tsx` | `local/spacing-scale` | 2 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-goals-goal-card'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-goals-goal-card'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-goals-goal-card'`, and it is a veto
