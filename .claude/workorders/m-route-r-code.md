...
# Work order: m-route-r-code

## Goal

Bring `m-route-r-code` (`/r/[code]`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-r-code'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-r-code'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-r-code'`, and it is a veto
