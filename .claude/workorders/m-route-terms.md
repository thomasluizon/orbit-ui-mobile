...
# Work order: m-route-terms

## Goal

Bring `m-route-terms` (`/terms`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
| file | rule | count |
|---|---|---|
| `apps/mobile/app/terms.tsx` | `local/spacing-scale` | 2 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-terms'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-terms'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-terms'`, and it is a veto
