---
surfaceId: residual-web-app-(auth)
platform: web
kind: residual
ownedFiles: 1
cells: 0
...
# Work order: residual-web-app-(auth)
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(auth)/error.tsx` | `local/spacing-scale` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'residual-web-app-(auth)'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'residual-web-app-(auth)'` agrees.
