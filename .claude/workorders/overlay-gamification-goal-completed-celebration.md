---
surfaceId: overlay-gamification-goal-completed-celebration
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-gamification-goal-completed-celebration

## Goal

Bring `overlay-gamification-goal-completed-celebration` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/gamification/goal-completed-celebration.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/gamification/goal-completed-celebration.tsx` | `local/no-arbitrary-zindex` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-gamification-goal-completed-celebration'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-gamification-goal-completed-celebration'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-gamification-goal-completed-celebration'`, and it is a veto
