---
surfaceId: overlay-components-friend-profile-view
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-components-friend-profile-view

## Goal

Bring `overlay-components-friend-profile-view` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(app)/social/_components/friend-profile-view.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(app)/social/_components/friend-profile-view.tsx` | `local/spacing-scale` | 10 |
...
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-components-friend-profile-view'` agrees.
