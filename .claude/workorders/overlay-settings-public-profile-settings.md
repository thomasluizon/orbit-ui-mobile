---
surfaceId: overlay-settings-public-profile-settings
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-settings-public-profile-settings

## Goal

Bring `overlay-settings-public-profile-settings` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/settings/public-profile-settings.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/settings/public-profile-settings.tsx` | `local/spacing-scale` | 4 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-settings-public-profile-settings'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-settings-public-profile-settings'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-settings-public-profile-settings'`, and it is a veto
