...
# Work order: route-wrapped

## Goal

Bring `route-wrapped` (`/wrapped`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(app)/wrapped/_components/wrapped-cover.tsx`
- `apps/web/app/(app)/wrapped/_components/wrapped-player.tsx`
- `apps/web/app/(app)/wrapped/_components/wrapped-slide.tsx`
- `apps/web/app/(app)/wrapped/_components/wrapped-styles.ts`
- `apps/web/app/(app)/wrapped/page.tsx`

## Backlog A: enumerated and machine-COUNTED (the fix is still a judgement call)

9 suppressed DESIGN.md violation(s) in your owned files. These are real defects that
were measured and committed to the lint baseline, then never assigned to anyone.

| file | rule | count |
|---|---|---|
| `apps/web/app/(app)/wrapped/_components/wrapped-slide.tsx` | `local/spacing-scale` | 5 |
| `apps/web/app/(app)/wrapped/_components/wrapped-cover.tsx` | `local/spacing-scale` | 2 |
| `apps/web/app/(app)/wrapped/_components/wrapped-player.tsx` | `local/spacing-scale` | 2 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'route-wrapped'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'route-wrapped'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'route-wrapped'`, and it is a veto
