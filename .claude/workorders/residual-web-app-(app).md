---
surfaceId: residual-web-app-(app)
platform: web
kind: residual
ownedFiles: 2
cells: 0
...
# Work order: residual-web-app-(app)
...
- `apps/web/app/(app)/today-sections.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(app)/today-sections.tsx` | `local/spacing-scale` | 8 |
| `apps/web/app/(app)/error.tsx` | `local/spacing-scale` | 3 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'residual-web-app-(app)'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'residual-web-app-(app)'` agrees.
