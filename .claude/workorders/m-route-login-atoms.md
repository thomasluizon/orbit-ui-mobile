...
# Work order: m-route-login-atoms

## Goal

Bring `m-route-login-atoms` (`/login-atoms`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/login-atoms.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-login-atoms'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-login-atoms'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-login-atoms'`, and it is a veto
