...
| file | rule | count |
|---|---|---|
| `apps/web/components/habits/habit-row.tsx` | `local/spacing-scale` | 3 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-habits-habit-row'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-habits-habit-row'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-habits-habit-row'`, and it is a veto
