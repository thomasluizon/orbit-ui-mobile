---
surfaceId: m-route-social-challenges-id
platform: mobile
kind: route
href: /social/challenges/[id]
ownedFiles: 1
cells: 4
mechanicalDebt: 0
pixelEvidence: none
...
# Work order: m-route-social-challenges-id

## Goal

Bring `m-route-social-challenges-id` (`/social/challenges/[id]`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/social/challenges/[id].tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-social-challenges-id'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-social-challenges-id'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-social-challenges-id'`, and it is a veto
