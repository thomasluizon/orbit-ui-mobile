---
surfaceId: residual-web-components-today
platform: web
kind: residual
ownedFiles: 1
cells: 0
...
# Work order: residual-web-components-today
...
- `apps/web/components/today/setup-checklist-card.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/today/setup-checklist-card.tsx` | `local/spacing-scale` | 3 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'residual-web-components-today'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'residual-web-components-today'` agrees.
