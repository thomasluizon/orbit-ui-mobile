---
surfaceId: m-route-retrospective-no-data-state
platform: mobile
kind: route
href: /retrospective-no-data-state
ownedFiles: 1
cells: 4
mechanicalDebt: 0
pixelEvidence: none
...
# Work order: m-route-retrospective-no-data-state

## Goal

Bring `m-route-retrospective-no-data-state` (`/retrospective-no-data-state`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/retrospective-no-data-state.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-retrospective-no-data-state'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-retrospective-no-data-state'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-retrospective-no-data-state'`, and it is a veto
