...
# Work order: m-route-email-step

## Goal

Bring `m-route-email-step` (`/email-step`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/email-step.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-email-step'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-email-step'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-email-step'`, and it is a veto
