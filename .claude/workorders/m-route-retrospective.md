---
surfaceId: m-route-retrospective
platform: mobile
kind: route
href: /retrospective
ownedFiles: 1
...
# Work order: m-route-retrospective

## Goal

Bring `m-route-retrospective` (`/retrospective`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/retrospective.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-retrospective'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-retrospective'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-retrospective'`, and it is a veto
