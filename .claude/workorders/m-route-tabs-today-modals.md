---
surfaceId: m-route-tabs-today-modals
platform: mobile
kind: route
href: /today-modals
ownedFiles: 1
...
# Work order: m-route-tabs-today-modals

## Goal

Bring `m-route-tabs-today-modals` (`/today-modals`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/mobile/app/(tabs)/today-modals.tsx`
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-tabs-today-modals'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-tabs-today-modals'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-tabs-today-modals'`, and it is a veto
