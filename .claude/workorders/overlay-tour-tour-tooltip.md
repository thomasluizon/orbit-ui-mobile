---
surfaceId: overlay-tour-tour-tooltip
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-tour-tour-tooltip

## Goal

Bring `overlay-tour-tour-tooltip` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/tour/tour-tooltip.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/tour/tour-tooltip.tsx` | `local/spacing-scale` | 3 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-tour-tour-tooltip'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-tour-tour-tooltip'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-tour-tour-tooltip'`, and it is a veto
