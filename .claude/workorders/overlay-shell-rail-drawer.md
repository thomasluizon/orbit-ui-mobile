---
surfaceId: overlay-shell-rail-drawer
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-shell-rail-drawer

## Goal

Bring `overlay-shell-rail-drawer` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/components/shell/rail-drawer.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/shell/rail-drawer.tsx` | `local/no-arbitrary-zindex` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-shell-rail-drawer'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-shell-rail-drawer'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-shell-rail-drawer'`, and it is a veto
