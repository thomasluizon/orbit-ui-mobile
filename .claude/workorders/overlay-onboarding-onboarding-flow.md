---
surfaceId: overlay-onboarding-onboarding-flow
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-onboarding-onboarding-flow

## Goal

Bring `overlay-onboarding-onboarding-flow` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
| file | rule | count |
|---|---|---|
| `apps/web/components/onboarding/onboarding-flow.tsx` | `local/spacing-scale` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-onboarding-onboarding-flow'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-onboarding-onboarding-flow'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-onboarding-onboarding-flow'`, and it is a veto
