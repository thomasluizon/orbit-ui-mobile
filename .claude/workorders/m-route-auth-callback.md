---
surfaceId: m-route-auth-callback
platform: mobile
kind: route
href: /auth-callback
ownedFiles: 1
...
# Work order: m-route-auth-callback

## Goal

Bring `m-route-auth-callback` (`/auth-callback`) to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'm-route-auth-callback'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'm-route-auth-callback'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'm-route-auth-callback'`, and it is a veto
