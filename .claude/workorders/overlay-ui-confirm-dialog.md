---
surfaceId: overlay-ui-confirm-dialog
platform: web
kind: overlay
ownedFiles: 1
cells: 4
...
# Work order: overlay-ui-confirm-dialog

## Goal

Bring `overlay-ui-confirm-dialog` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
| file | rule | count |
|---|---|---|
| `apps/web/components/ui/confirm-dialog.tsx` | `local/spacing-scale` | 6 |
| `apps/web/components/ui/confirm-dialog.tsx` | `local/no-arbitrary-zindex` | 1 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-ui-confirm-dialog'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-ui-confirm-dialog'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-ui-confirm-dialog'`, and it is a veto
