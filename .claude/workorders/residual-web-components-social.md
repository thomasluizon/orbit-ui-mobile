---
surfaceId: residual-web-components-social
platform: web
kind: residual
ownedFiles: 1
cells: 0
...
# Work order: residual-web-components-social
...
- `apps/web/components/social/social-entry-card.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/components/social/social-entry-card.tsx` | `local/spacing-scale` | 4 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'residual-web-components-social'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'residual-web-components-social'` agrees.
