---
surfaceId: overlay-app-today-page-view
platform: web
kind: overlay
ownedFiles: 1
cells: 8
...
Bring `overlay-app-today-page-view` to DESIGN.md. Read DESIGN.md once, then edit; the parts that apply to this surface are named below so you do not have to search for them.
...
- `apps/web/app/(app)/today-page-view.tsx`
...
| file | rule | count |
|---|---|---|
| `apps/web/app/(app)/today-page-view.tsx` | `local/spacing-scale` | 2 |
...
1. Backlog A is 0 (`node tools/workorder.mjs --check --id 'overlay-app-today-page-view'` exits 0).
2. The diff touches only the owned files above: `tools/check-diff-ownership.mjs --id 'overlay-app-today-page-view'` agrees.
...
surface comes from `node tools/workorder.mjs --check --id 'overlay-app-today-page-view'`, and it is a veto
