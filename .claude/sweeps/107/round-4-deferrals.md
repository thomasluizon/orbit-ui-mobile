...
### DEF-R7-A11Y-STATUS-TINT — parity addendum (final)

The web amber-on-tint TEXT sites (conflict-warning MEDIUM, trial-banner lastDay + upgrade link, upgrade trial block) were migrated base `--status-overdue` → `--status-overdue-text` to match their mobile twins and gain the strict improvement (light 2.75 → 4.06). They remain in this deferral family (still < 4.5 light = the amber-on-amber-tint token ceiling; all pass in dark). Now every member of the family uses the `-text` token on both platforms — web↔mobile parity holds. The residual sub-4.5 is the DESIGN.md token-ceiling decision described above; non-color cues (icon + literal status wording) satisfy WCAG 1.4.1.
