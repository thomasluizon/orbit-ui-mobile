...
Workflow({ scriptPath: '.claude/workflows/prod-readiness.mjs', args: {
  scope: '<resolved {scope}>', roots: { ui: '<resolved ui>', api: '<resolved api>' }, performanceMeasurement: productionMeasurement
} })
```
...
`architecture.mmd` from the tree, none of them committed (#470). It scans endpoint usage across
`apps/web`, walks the symbol-filtered import closure for i18n ownership, and refuses to guess
...
- A real, intentional platform divergence (e.g. web `/u/[slug]` is the public view a shared
  ...
  row green: an alias pairs two files only when they are the SAME screen.
