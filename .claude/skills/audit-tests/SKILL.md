...
The "intelligent test" rubric in `rubric.md` comes from the test-quality bases on
https://claudeskills.info plus the behavior-over-implementation canon, specialized to
Orbit's reality: **unit-tests-only** (the integration/E2E suites were removed in both
repos), Vitest on the frontend, xUnit + FluentAssertions on the backend, and the critical
paths that carry the risk here (auth, billing, AI tools). The workflow's finder/skeptic
agents *read* local test + source files and run `git` / `rg`; running the suite is
`/validate`'s job, never this skill's.
...
`TESTING.md` (repo root) is the canonical suite catalog: where each suite lives and what it proves.
...
out-of-policy (it was deliberately removed), don't reward it. The one sanctioned E2E is the
separate ~5-test web smoke suite (#227), out of scope here.
...
```
Workflow({ scriptPath: '.claude/workflows/audit.mjs', args: { kind: 'tests', scope: '<resolved {scope}>', roots: { ui, api } } })
```
...
- **Score by count or coverage %, or file a coverage-percentage finding.** The green bar is
  the mechanical layer (D11) and a green bar over rubber-stamp tests is the exact failure
  this audit exists to catch. Judge by what a test would *fail on*.
