...
5. Append each qualifying candidate to `.claude/pending-lessons.md`. Use this exact shape without
 quotes, dates, session IDs or attribution:
   ```text
 ## <one-line lesson>
 - Trigger: <task/files>
   ...
   ```
 Keep the timed evidence and session IDs in the scratchpad outside the repository.
