...
This local build always uses Google TEST AdMob ad units (never real ads), so QA installs never serve or click live ads. Real-ad production releases ship only through the `.github/workflows/android-release.yml` CI workflow, which injects the real AdMob IDs and guards against shipping test units.
