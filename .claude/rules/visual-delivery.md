...
### 4. Done = an artifact per surface, not a sentence
For each surface on the inventory, "done" means a **screenshot of the rendered surface, seeded, in light AND dark**, that a reviewer can spot-check against `DESIGN.md` and the mockups (`KQMPM`/`N8aEDF`). No screenshot → not done. The artifact is the unit of completion; the checklist row flips only when its image exists. On a PR, attach or link the artifacts so 40 thumbnails can be scanned in a minute instead of a paragraph being trusted.
