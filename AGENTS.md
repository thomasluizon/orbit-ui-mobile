...
Frontend design rules live in **`DESIGN.md`** at the repo root. Read it before any frontend work — it is the source of truth for Orbit's tokens, primitives, motion, and bans.

- The anchor is **Linear-tactical**, locked. Do not re-pick per session, do not hybridise.
...
- v8 OKLCH tokens (`--bg`, `--fg-1`, `--primary`, `--hairline`, etc.) are canonical. Do **not** introduce `--color-background`, `bg-surface-*`, or `text-text-*` in new code — they are aliased for legacy compat only and will be removed.
- Mobile-first. The canvas is a 412px phone shell on both platforms (web caps at `--app-max-w: 640px`).
