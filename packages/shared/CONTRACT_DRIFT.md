...
regenerates with the lockfile's orval version and updates the pin. The `publish`
job opens or updates the single `chore/contract-snapshot` pull request against
`main`. It never pushes to `main`.
