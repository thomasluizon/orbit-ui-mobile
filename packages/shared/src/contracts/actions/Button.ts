 ...
 * Fab's `label` is. A control whose only child is a glyph has no text node to name it, so
 * `iconOnly` pairs with a REQUIRED `label` and neither can be passed without the other.
 */
