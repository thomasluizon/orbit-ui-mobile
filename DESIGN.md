...
are not reachable from the chat surface.** API-key management and account deletion are the only
step-up operations in the product. Billing is a provider handoff from the Subscription flow under
Profile. An assistant that can sell a subscription contradicts the voice pillar "describe, never
sell". API-key management and account deletion stay available on the MCP surface, where an external
