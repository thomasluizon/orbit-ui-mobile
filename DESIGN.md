...
**D42 covers `design/reference.html` AND the granted canvas export** (amended 2026-08-25). A rendered artefact outranks this prose only when it is the artefact Thomas granted, and `## Information architecture` still outranks every export on whether a surface should exist at all.

**Thomas granted the twenty-one-screen Claude Design export on 2026-08-25**, and it is committed at `design/canvas/` with the design system's 166 token values at `design/canvas/_ds/orbit-design-system-918bd5d7-839c-4dd0-811b-4a8781f60507/`. That export is authoritative for every redesign surface: **where a drawing there and this prose disagree, the drawing wins and this prose is the defect.** A number typed into a component that disagrees with a token in `_ds/tokens/` is wrong, whatever this document says. Read the drawing for the surface you are building before you read the section here that describes it.

An export Thomas has NOT granted still carries no authority. The eleven documents from the pass that predates `## Information architecture` are quarantined in `design/canvas/superseded/` for exactly that reason: they draw a habit tracker with a chat tab, they still show Goals and Retrospective as separate screens, and one of them ships a habit-limit figure that exists nowhere in the code. Never build from them.
...
--status-skip     var(--fg-3)               /* PRE-REDESIGN ROWS ONLY. See the note below */
...
- **A habit row is never frozen and never skipped** (traced from source 2026-08-17). `StreakFreeze` is `(UserId, UsedOnDate)`, so a freeze marks a **day** for a **user** and `Habit` carries no freeze member at all. Frozen renders on the streak day strip inside Progresso and nowhere else. Skipping advances the schedule, so the row leaves the day rather than taking a status; only a flexible habit even writes a log row for it. `--status-frozen` therefore serves the day strip only, and `--status-skip` binds to no habit-row state. This is the specification that produced a frozen habit on the first composed screen, so it is stated as a prohibition rather than an omission. `--status-skip` is kept in the table anyway, because three shipping components still read it (`apps/web/components/habits/habit-row-check-circle.tsx:12`, `apps/web/components/ui/status-dot.tsx:28`, `apps/web/components/habits/bulk-action-bar-v2.tsx:163`) and deleting a token those files read breaks the live app before the redesign replaces them. It is deleted with the row work in the R3 and R5 tickets, not before, and no new surface may use it (2026-08-20).
